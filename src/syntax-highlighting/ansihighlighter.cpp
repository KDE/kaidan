...
AnsiHighlighter::AnsiHighlighter()
    : AbstractHighlighter()
    , d(std::make_unique<AnsiHighlighterPrivate>())
{
}
...
void AnsiHighlighter::setOutputString(QString *string)
{
    d->out.setString(string);
}

void AnsiHighlighter::highlightData(QIODevice *dev, AnsiFormat format, Options options)
{
    if (!d->out.device() && !d->out.string()) {
        qFatal("[AnsiHighlighter] No output stream defined!");
        return;
    }
