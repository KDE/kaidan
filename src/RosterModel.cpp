...
void RosterModel::replaceItems(const QHash<QString, RosterItem> &items)
{
    ...
    for (auto item : items) {
        // find old item
        auto oldItem = std::find_if(m_items.cbegin(), m_items.cend(), [&](const RosterItem &oldItem) {
...
void RosterModel::updateLastMessage(QVector<RosterItem>::Iterator &itr, const Message &message, QVector<int> &changedRoles, bool onlyUpdateIfNewerOrAtSameAge)
{
    ...
    // is no draft message.
    if (const auto lastMessage = message.previewText(); itr->lastMessageDeliveryState != Enums::DeliveryState::Draft && itr->lastMessage != lastMessage) {
