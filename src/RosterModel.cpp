	...
	if (const auto itemBeingUnpinned = findItem(jid)) {
		for (const auto &item : std::as_const(m_items)) {
			...
			if (item.pinningPosition > itemBeingUnpinned->pinningPosition) {
				RosterDb::instance()->updateItem(item.jid,
		...
		RosterDb::instance()->updateItem(
			jid, [](RosterItem &item) { item.pinningPosition = -1; });
	}
}

void RosterModel::reorderPinnedItem(const QString &, const QString &, int oldIndex, int newIndex)
{
	if (oldIndex < 0 || oldIndex >= m_items.count() || newIndex < 0 ||
		newIndex >= m_items.count() || oldIndex == newIndex) {
		return;
	}
	...
	for (int i = min; i <= max; ++i) {
		...
		RosterDb::instance()->updateItem(
			item.jid, [movedUpwards, i, oldIndex, pinningPosition](RosterItem &item) {
				if (i == oldIndex) {
...
{
	RosterDb::instance()->updateItem(jid,
...
{
	RosterDb::instance()->updateItem(
		jid, [=](RosterItem &item) { item.notificationRule = notificationRule; });
}
...
void RosterModel::updateItem(const RosterItem &item)
{
	for (int i = 0; i < m_items.size(); i++) {
		...
		if (const auto oldItem = m_items.at(i); oldItem.jid == jid) {
			...
			// TODO: Current problem: last message sender JID is not cached and check with
			// lastMessageGroupChatSenderName may result in a conflict if a group chat user has the same name as a roster item
	...
	for (int i = 0; i < m_items.size(); i++) {
		...
		if (item.accountJid == accountJid && item.jid == jid) {
			...
			if (accountJid == ChatController::instance()->accountJid() &&
				jid == ChatController::instance()->chatJid()) {
...
QFuture<QVector<int>> RosterModel::updateLastMessage(QVector<RosterItem>::Iterator &itr,
	...
	bool onlyUpdateIfNewerOrAtSameAge)
{
	...
	if (const auto lastMessage = message.previewText();
		(itr->lastMessage != lastMessage || itr->lastMessageDateTime != message.timestamp) &&
		itr->lastMessageDeliveryState != Enums::DeliveryState::Draft) {
		itr->lastMessageDateTime = message.timestamp;
...
{
	static const QVector<int> changedRoles = {
		int(LastMessageDateTimeRole), int(LastMessageRole), int(LastMessageIsDraftRole)
	};
