...
QString QmlUtils::downloadPath(const QString &filename)
{
    ...
    const QDir directory(QStandardPaths::writableLocation(QStandardPaths::DownloadLocation) % QDir::separator() % QStringLiteral(APPLICATION_DISPLAY_NAME));
...
QString QmlUtils::processMsgFormatting(const QStringList &list, bool isFirst)
{
    ...
    if (list.first().startsWith(QStringLiteral("https://")) || list.first().startsWith(QStringLiteral("http://")))
        return (isFirst ? QString() : QStringLiteral(" ")) + QStringLiteral("<a href='%1'>%1</a>").arg(list.first()) + processMsgFormatting(list.mid(1), false);
