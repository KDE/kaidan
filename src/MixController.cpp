...
MixController::MixController(QObject *parent) : QObject(parent)
{
	...
	callRemoteTask(
		...
		[serviceJid](QXmppMixManager::CreationResult &&result) {
			if (const auto error = std::get_if<QXmppError>(&result)) {
				...
				Q_EMIT GroupChatController::instance()->privateGroupChatCreationFailed(
					serviceJid, errorMessage);
			} else {
				const auto channelJid = std::get<QXmppMixManager::ChannelJid>(result);
				Q_EMIT GroupChatController::instance()->groupChatCreated(
	...
	callRemoteTask(
		...
		[this, channelJid]() {
			return std::pair { Kaidan::instance()
						   ...
						   ->xmppClient()
						   ->findExtension<QXmppMixManager>()
						   ->createChannel(QXmppUtils::jidToDomain(channelJid),
			...
			if (const auto error = std::get_if<QXmppError>(&result)) {
				...
				if (error->isStanzaError()) {
					...
					case QXmppStanza::Error::Cancel:
						if (stanzaError->condition() == QXmppStanza::Error::Conflict ||
							stanzaError->condition() == QXmppStanza::Error::NotAllowed)
							errorMessage =
								tr("The group already exists");
						break;
					default:
						errorMessage = stanzaError->text();
					}
				...
				Q_EMIT GroupChatController::instance()->publicGroupChatCreationFailed(
					channelJid, errorMessage);
			} else {
				Q_EMIT GroupChatController::instance()->groupChatCreated(
					Kaidan::instance()->client()->xmppClient()->configuration().jidBare(),
					channelJid);
...
void MixController::requestChannelAccessibility(const QString &channelJid)
{
	callRemoteTask(
		...
		[channelJid](QXmppMixManager::ChannelJidResult &&result) {
			...
			} else if (const auto channelJids =
					   std::get<QVector<QXmppMixManager::ChannelJid>>(result);
				channelJids.contains(channelJid)) {
				GroupChatController::instance()->groupChatMadePublic(
					Kaidan::instance()->client()->xmppClient()->configuration().jidBare(),
					channelJid);
			} else {
				GroupChatController::instance()->groupChatMadePrivate(
					Kaidan::instance()->client()->xmppClient()->configuration().jidBare(),
					channelJid);
			}
		});
	...
	callRemoteTask(
		...
		[this, channelJid]() {
			return std::pair { Kaidan::instance()
						   ...
						   ->xmppClient()
						   ->findExtension<QXmppMixManager>()
						   ->requestChannelInformation(channelJid),
				this };
		},
		this,
	...
	callRemoteTask(
		...
		[this, channelJid, nickname]() {
			return std::pair { Kaidan::instance()
						   ...
						   ->xmppClient()
						   ->findExtension<QXmppMixManager>()
						   ->joinChannel(channelJid, nickname),
				this };
		},
		this,
		[channelJid](QXmppMixManager::JoiningResult &&result) {
			if (const auto error = std::get_if<QXmppError>(&result)) {
				...
				if (error->isStanzaError()) {
					...
					case QXmppStanza::Error::Cancel:
						...
						case QXmppStanza::Error::ItemNotFound:
							errorMessage =
								tr("The group does not exist");
							break;
						case QXmppStanza::Error::NotAllowed:
							errorMessage =
								tr("You aren't allowed to join the "
								   "group");
							break;
						default:
							break;
				...
				Q_EMIT GroupChatController::instance()->groupChatJoiningFailed(
					channelJid, errorMessage);
			} else {
				Q_EMIT GroupChatController::instance()->groupChatJoined(
					Kaidan::instance()->client()->xmppClient()->configuration().jidBare(),
					channelJid);
			}
		});
		...
		callRemoteTask(
			...
			[this, channelJid]() {
				return std::pair { Kaidan::instance()
							   ...
							   ->xmppClient()
							   ->findExtension<QXmppMixManager>()
							   ->requestChannelNodes(channelJid),
					this };
			},
			this,
			[this, sendInvitation, message, channelJid, contactJid](
				QXmppMixManager::ChannelNodeResult &&result) {
				if (const auto error = std::get_if<QXmppError>(&result)) {
					Kaidan::instance()->passiveNotificationRequested(
							...
							[this, channelJid, contactJid]() {
								return std::pair {
									Kaidan::instance()
										...
										->xmppClient()
										->findExtension<QXmppMixManager>()
										->allowJid(channelJid, contactJid),
									this
								};
							},
							this,
							[sendInvitation, message, channelJid, contactJid](
								QXmppClient::EmptyResult &&result) {
								if (const auto error = std::get_if<QXmppError>(
									    &result)) {
									Kaidan::instance()->passiveNotificationRequested(tr(
										"%1 could not be "
										"invited to %2: %3")
											.arg(contactJid,
												channelJid,
									...
									// to domains which are only used to restrict the channel's membership to JIDs of that domain.
									if (!QXmppUtils::jidToUser(contactJid)
											.isEmpty()) {
										sendInvitation();
									}
								}
				...
				if (nodes.testFlag(QXmppMixConfigItem::Node::AllowedJids)) {
					callRemoteTask(
						...
						[this, channelJid](QXmppMixManager::JidResult &&result) {
							if (const auto error = std::get_if<QXmppError>(&result)) {
								Kaidan::instance()->passiveNotificationRequested(tr(
									"Allowed users of %1 could "
									"not be retrieved: %2")
										.arg(channelJid, error->description));
							} else {
								const auto jids =
									std::get<QVector<QXmppMixManager::Jid>>(
										result);
	...
	callRemoteTask(
		...
		[this, channelJid]() {
			return std::pair { Kaidan::instance()
						   ...
						   ->xmppClient()
						   ->findExtension<QXmppMixManager>()
						   ->requestParticipants(channelJid),
				this };
		},
		this,
		...
		manager->requestChannelConfiguration(channelJid).then(client, [channelJid, userJid, client, manager](QXmppMixManager::ConfigurationResult &&result) {
			if (const auto error = std::get_if<QXmppError>(&result)) {
				...
				if (channelConfiguration.nodes().testFlag(QXmppMixConfigItem::Node::BannedJids)) {
					...
					channelConfiguration.setNodes(channelConfiguration.nodes() |
								      QXmppMixConfigItem::Node::BannedJids);
					manager->updateChannelConfiguration(channelJid, channelConfiguration)
						.then(client, [client, channelJid, banJid](auto) {
							Kaidan::instance()
								->client()
								->xmppClient()
								->findExtension<QXmppMixManager>()
								->updateSubscriptions(channelJid,
									{ QXmppMixConfigItem::Node::BannedJids })
								.then(client,
									[banJid](auto) { banJid(); });
						});
...
void MixController::leaveChannel(const QString &channelJid)
{
	callRemoteTask(
		...
		[channelJid](QXmppClient::EmptyResult &&result) {
			...
			if (const auto error = std::get_if<QXmppError>(&result)) {
				Q_EMIT GroupChatController::instance()->groupChatLeavingFailed(
					ownJid, channelJid, error->description);
			} else {
	...
	callRemoteTask(
		...
		[this, channelJid](QXmppClient::EmptyResult &&result) {
			if (const auto error = std::get_if<QXmppError>(&result)) {
				...
				if (error->isStanzaError()) {
					...
					case QXmppStanza::Error::Cancel:
						if (stanzaError->condition() == QXmppStanza::Error::NotAllowed)
							errorMessage =
								tr("You aren't allowed to delete "
								   "the group");
						break;
					default:
						errorMessage = stanzaError->text();
				...
				Q_EMIT GroupChatController::instance()->groupChatDeletionFailed(
					Kaidan::instance()->client()->xmppClient()->configuration().jidBare(),
					channelJid,
					errorMessage);
			} else {
...
void MixController::handleParticipantSupportChanged()
{
	runOnThread(
		...
		[this](QXmppMixManager::Support participantSupport) {
			m_channelParticipationSupported =
				(participantSupport == QXmppMixManager::Support::Supported);
...
{
	if (isPublic)
		Q_EMIT GroupChatController::instance()->groupChatMadePublic(
			Kaidan::instance()->client()->xmppClient()->configuration().jidBare(), channelJid);
	else
		Q_EMIT GroupChatController::instance()->groupChatMadePrivate(
			Kaidan::instance()->client()->xmppClient()->configuration().jidBare(), channelJid);
}
...
void MixController::handleChannelMadePrivate(const QString &channelJid)
{
	runOnThread(Kaidan::instance()->client(), [channelJid]() {
		Kaidan::instance()->client()->xmppClient()->findExtension<QXmppMixManager>()->updateSubscriptions(
			channelJid);
	});
...
{
	Q_EMIT GroupChatController::instance()->groupChatDeleted(
		Kaidan::instance()->client()->xmppClient()->configuration().jidBare(), channelJid);
}
