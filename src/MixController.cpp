...
{
	callRemoteTask(
		Kaidan::instance()->client(), [this, serviceJid]() {
...
{
	callRemoteTask(
		Kaidan::instance()->client(), [this, channelJid]() {
			return std::pair { Kaidan::instance()->client()->xmppClient()->findExtension<QXmppMixManager>()->createChannel(QXmppUtils::jidToDomain(channelJid), QXmppUtils::jidToUser(channelJid)), this };
...
void MixController::requestChannelInformation(const QString &channelJid)
{
	callRemoteTask(
		...
		}, this, [this, channelJid](QXmppMixManager::InformationResult &&result) {
			if (const auto error = std::get_if<QXmppError>(&result)) {
				Q_EMIT Kaidan::instance()->passiveNotificationRequested(tr("Could not retrieve information of group %1: %2").arg(channelJid, error->description));
			} else {
...
{
	callRemoteTask(
		Kaidan::instance()->client(), [this, channelJid, nickname]() {
...
void MixController::inviteContactToChannel(const QString &channelJid, const QString &contactJid, bool channelPublic)
{
	...
	if (channelPublic) {
		...
		callRemoteTask(
			...
			}, this, [this, sendInvitation, message, channelJid, contactJid](QXmppMixManager::ChannelNodeResult &&result) {
				if (const auto error = std::get_if<QXmppError>(&result)) {
					...
					auto allowJid = [this, sendInvitation, message, channelJid, contactJid]() {
						callRemoteTask(
							Kaidan::instance()->client(), [this, channelJid, contactJid]() {
								return std::pair { Kaidan::instance()->client()->xmppClient()->findExtension<QXmppMixManager>()->allowJid(channelJid, contactJid), this };
							}, this, [sendInvitation, message, channelJid, contactJid](QXmppClient::EmptyResult &&result) {
								if (const auto error = std::get_if<QXmppError>(&result)) {
									Kaidan::instance()->passiveNotificationRequested(tr("%1 could not be invited to %2: %3").arg(contactJid, channelJid, error->description));
								} else {
...
void MixController::requestChannelUsers(const QString &channelJid)
{
	callRemoteTask(
		Kaidan::instance()->client(), [this, channelJid]() {
			return std::pair { Kaidan::instance()->client()->xmppClient()->findExtension<QXmppMixManager>()->requestChannelNodes(channelJid), this };
		}, this, [this, channelJid](QXmppMixManager::ChannelNodeResult &&result) {
			if (const auto error = std::get_if<QXmppError>(&result)) {
				...
				if (nodes.testFlag(QXmppMixConfigItem::Node::AllowedJids)) {
					callRemoteTask(
						Kaidan::instance()->client(), [this, channelJid]() {
							return std::pair { Kaidan::instance()->client()->xmppClient()->findExtension<QXmppMixManager>()->requestAllowedJids(channelJid), this };
						}, this, [this, channelJid](QXmppMixManager::JidResult &&result) {
							if (const auto error = std::get_if<QXmppError>(&result)) {
								Kaidan::instance()->passiveNotificationRequested(tr("Allowed users of %1 could not be retrieved: %2").arg(channelJid, error->description));
							} else {
				...
				if (nodes.testFlag(QXmppMixConfigItem::Node::BannedJids)) {
					callRemoteTask(
						Kaidan::instance()->client(), [this, channelJid]() {
							...
							if (const auto error = std::get_if<QXmppError>(&result)) {
								Kaidan::instance()->passiveNotificationRequested(tr("Banned users of %1 could not be retrieved: %2").arg(channelJid, error->description));
							} else {
								const auto jids = std::get<QVector<QXmppMixManager::Jid>>(result);
								for (const auto &jid : jids) {
	...
	callRemoteTask(
		...
		}, this, [this, channelJid](QXmppMixManager::ParticipantResult result) {
			if (const auto error = std::get_if<QXmppError>(&result)) {
...
void MixController::deleteChannel(const QString &channelJid)
{
	callRemoteTask(
		...
		}, this, [this, channelJid](QXmppClient::EmptyResult &&result) {
			if (const auto error = std::get_if<QXmppError>(&result)) {
				QString errorMessage;
...
void MixController::handleParticipantSupportChanged()
{
	...
	}, this, [this](QXmppMixManager::Support participantSupport) {
...
void MixController::handleServicesChanged()
{
	...
	}, this, [this](const QList<QXmppMixManager::Service> &&services) {
		m_services = services;
