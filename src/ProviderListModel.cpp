...
void ProviderListModel::readItemsFromJsonFile(const QString &filePath)
{
    QFile file(filePath);
    if (!file.exists()) {
        qCWarning(KAIDAN_LOG) << "[ProviderListModel] Could not parse provider list:" << filePath << "- file does not exist!";
        return;
    }

    if (!file.open(QIODevice::ReadOnly)) {
        qCWarning(KAIDAN_LOG) << "[ProviderListModel] Could not open file for reading:" << filePath;
        return;
    }
    ...
    if (jsonProviderArray.isEmpty()) {
        qCWarning(KAIDAN_LOG) << "[ProviderListModel] Could not parse provider list JSON file or no providers defined.";
        qCWarning(KAIDAN_LOG) << "[ProviderListModel] QJsonParseError:" << parseError.errorString() << "at" << parseError.offset;
        return;
    }
