...
int ProviderListModel::randomlyChooseIndex(const QList<int> &excludedIndexes, bool providersMatchingSystemLocaleOnly) const
{
	...
	if (providersMatchingSystemLocaleOnly) {
		...
		return indexOfRandomlySelectedProvider(
			providersWithInBandRegistrationAndSystemLocale, excludedIndexes);
	}
	...
	const QVector<ProviderListItem> &preSelectedProviders) const
{
	...
	for (const auto &provider : preSelectedProviders) {
		if (const auto systemLocale = SystemUtils::systemLocaleCodes();
			provider.languages().contains(systemLocale.languageCode) &&
...
int ProviderListModel::indexOfRandomlySelectedProvider(const QVector<ProviderListItem> &preSelectedProviders,
	const QList<int> &excludedIndexes) const
{
	int index;

	do {
		index = m_items.indexOf(preSelectedProviders.at(
