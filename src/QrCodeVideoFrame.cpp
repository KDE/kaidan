...
uchar yuvToGray(uchar Y, uchar U, uchar V)
{
	...
	return gray(qBound<uchar>(0, uchar((298 * C + 409 * E + 128) >> 8), 255),
...
uchar yuvToGray2(uchar y, uchar u, uchar v)
{
	...
	return gray(qBound<uchar>(0, uchar(floor(rD)), 255),
		qBound<uchar>(0, uchar(floor(gD)), 255),
		qBound<uchar>(0, uchar(floor(bD)), 255));
}

static QImage *rgbDataToGrayscale(const uchar *data,
...
QImage *QrCodeVideoFrame::toGrayscaleImage()
{
	...
	case QVideoFrameFormat::PixelFormat::Format_NV12:
		...
		image = new QImage(captureRect.targetWidth, captureRect.targetHeight,
	QImage::Format_Grayscale8); pixel = image->bits(); wh = m_size.width() * m_size.height(); w_2
	= m_size.width() / 2; wh_54 = wh * 5 / 4;
	...
	case QVideoFrameFormat::PixelFormat::Format_NV21:
		...
		image = new QImage(data, captureRect.targetWidth, captureRect.targetHeight,
	QImage::Format_Grayscale8); break; case QVideoFrameFormat::PixelFormat::Format_YUYV: image =
	new QImage(captureRect.targetWidth, captureRect.targetHeight, QImage::Format_Grayscale8); pixel
	= image->bits();
