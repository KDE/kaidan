...
template<typename F>
static QStringList buildCodecList(const QStringList &codecs, F toString, MediaRecorder *recorder, const QString &startsWith = QString())
{
...
static QList<QSize> buildResolutionList(const QList<QSize> &resolutions)
{
    ...
    std::sort(entries.begin(), entries.end(), [](const QSize &left, const QSize &right) {
        return left.width() == right.width() ? left.height() < right.height() : left.width() < right.width();
    });
    ...
    , m_videoQualityModel(new MediaSettingsQualityModel(this, this))
{
    connect(this, &MediaRecorder::readyChanged, this, [this]() {
        ...
        case MediaRecorder::Type::Image: {
            ...
            m_imageResolutionModel->setValuesAndCurrentValue(buildResolutionList(m_imageCapturer->supportedResolutions()), m_imageEncoderSettings.resolution);
            m_imageQualityModel->setValuesAndCurrentValue(buildQualityList(), m_imageEncoderSettings.quality);
            break;
        }
        case MediaRecorder::Type::Audio: {
            ...
            m_audioQualityModel->setValuesAndCurrentValue(buildQualityList(), m_audioEncoderSettings.quality);
            break;
        }
        case MediaRecorder::Type::Video: {
            ...
            m_videoResolutionModel->setValuesAndCurrentValue(buildResolutionList(m_videoRecorder->supportedResolutions()), m_videoEncoderSettings.resolution);
            m_videoFrameRateModel->setValuesAndCurrentValue(buildFrameRateList(m_videoRecorder->supportedFrameRates()), m_videoEncoderSettings.frameRate);
            m_videoQualityModel->setValuesAndCurrentValue(buildQualityList(), m_videoEncoderSettings.quality);
            break;
        }
        }
...
bool MediaRecorder::isReady() const
{
    ...
    case MediaRecorder::Type::Audio:
        return isAvailable() && status() >= MediaRecorder::Status::UnloadedStatus && status() <= MediaRecorder::Status::LoadedStatus;
    case MediaRecorder::Type::Video:
        return isAvailable() && m_camera->status() == QCamera::Status::ActiveStatus && status() >= MediaRecorder::Status::UnloadedStatus
...
MediaRecorder::Error MediaRecorder::error() const
{
    ...
    static const QMap<int, MediaRecorder::Error> recorderMapping = {{QMediaRecorder::Error::NoError, MediaRecorder::Error::NoError},
...
QString MediaRecorder::encoderContainer(const QString &container, const void *userData)
{
    ...
    return container.isEmpty() ? tr("Default") : QString::fromLatin1("%1 (%2)").arg(mediaRecorder->containerDescription(container), container);
}

QString MediaRecorder::encoderResolution(const QSize &size, const void *userData)
{
    Q_UNUSED(userData);
    return size.isEmpty() ? tr("Default") : QString::fromLatin1("%1x%2").arg(QString::number(size.width()), QString::number(size.height()));
}
...
QString MediaRecorder::imageEncoderCodec(const QString &codec, const void *userData)
{
    ...
    return codec.isEmpty() ? tr("Default") : QString::fromLatin1("%1 (%2)").arg(capturer->imageCodecDescription(codec), codec);
}
...
QString MediaRecorder::audioEncoderCodec(const QString &codec, const void *userData)
{
    const auto *recorder = static_cast<const MediaRecorder *>(userData);
    const auto *mediaRecorder = recorder->m_audioRecorder ? recorder->m_audioRecorder : recorder->m_videoRecorder;
    return codec.isEmpty() ? tr("Default") : QString::fromLatin1("%1 (%2)").arg(mediaRecorder->audioCodecDescription(codec), codec);
}
...
QString MediaRecorder::videoEncoderCodec(const QString &codec, const void *userData)
{
    ...
    return codec.isEmpty() ? tr("Default") : QString::fromLatin1("%1 (%2)").arg(videoRecorder->videoCodecDescription(codec), codec);
}
