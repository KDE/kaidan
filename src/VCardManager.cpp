...
void VCardManager::requestVCard(const QString &jid)
{
    if (m_client->state() == QXmppClient::ConnectedState)
        ...
        qCWarning(KAIDAN_LOG) << "[VCardManager] Could not fetch vCard: Not connected to a server";
}
...
void VCardManager::requestClientVCard()
{
    if (m_client->state() == QXmppClient::ConnectedState)
        ...
        qCWarning(KAIDAN_LOG) << "[VCardManager] Could not fetch own vCard: Not connected to a server";
}
