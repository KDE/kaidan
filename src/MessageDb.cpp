...
#include "TrustDb.h"
#include "kaidan_messagedb_debug.h"
                                           ...
                                           const QList<EncryptedSource> &encryptedSources)
{
    return run([=, this] {
        ...
        if (msgs.empty()) {
            qCDebug(KAIDAN_MESSAGEDB_LOG) << "Could not find message with ID" << messageId << "to attach file sources.";
            return;
        }
        ...
        if (fileItr == message.files.end()) {
            qCDebug(KAIDAN_MESSAGEDB_LOG) << "Could not attach sources to message with ID" << messageId << ": No file with external ID of" << externalFileId
                                          << "found.";
            return;
        }
        auto &file = *fileItr;
