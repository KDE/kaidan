...
void RosterManager::addContact(const QString &jid, const QString &name, const QString &message, bool automaticInitialAddition)
{
    if (m_client->state() == QXmppClient::ConnectedState) {
        ...
        qCWarning(KAIDAN_LOG) << "[client] [RosterManager] Could not add contact, as a result of "
                                 "not being connected.";
    }
}
...
void RosterManager::removeContact(const QString &jid)
{
    if (m_client->state() == QXmppClient::ConnectedState) {
        ...
        qCWarning(KAIDAN_LOG) << "[client] [RosterManager] Could not remove contact, as a result of "
                                 "not being connected.";
    }
}
...
void RosterManager::renameContact(const QString &jid, const QString &newContactName)
{
    if (m_client->state() == QXmppClient::ConnectedState) {
        ...
        qCWarning(KAIDAN_LOG) << "[client] [RosterManager] Could not rename contact, as a result of "
                                 "not being connected.";
    }
}
