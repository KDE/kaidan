...
{
    return m_currentIndex >= 0 && m_currentIndex < m_cameras.count() ? CameraInfo(m_cameras[m_currentIndex]) : CameraInfo();
}
...
void CameraModel::refresh()
{
    ...
    const auto it = std::find_if(m_cameras.constBegin(), m_cameras.constEnd(), [&currentDeviceName](const QCameraDevice &deviceInfo) {
