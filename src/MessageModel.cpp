        ...
        if (m_messages.isEmpty()) {
            ...
            if (m_rosterItemWatcher.item().unreadMessages > 0) {
                ...
                if (lastReadContactMessageId.isEmpty()) {
                    MessageDb::instance()->fetchMessagesUntilFirstContactMessage(AccountManager::instance()->jid(), m_currentChatJid, 0);
                } else {
                    MessageDb::instance()->fetchMessagesUntilId(AccountManager::instance()->jid(), m_currentChatJid, 0, lastReadContactMessageId);
                }
            } else {
                MessageDb::instance()->fetchMessages(AccountManager::instance()->jid(), m_currentChatJid, 0);
            }
        } else {
            MessageDb::instance()->fetchMessages(AccountManager::instance()->jid(), m_currentChatJid, m_messages.size());
        }
                    ...
                    [=, this](QFuture<QXmpp::SendResult> future) {
                        await(future, this, [=, this](QXmpp::SendResult result) {
                            if (const auto error = std::get_if<QXmppError>(&result)) {
                                Q_EMIT Kaidan::instance()->passiveNotificationRequested(tr("Reaction could not be "
                                                                                           "sent: %1")
        ...
        if (ConnectionState(Kaidan::instance()->connectionState()) == Enums::ConnectionState::StateConnected) {
            ...
            runOnThread(
                ...
                [=, this](QFuture<QXmpp::SendResult> future) {
                    await(future, this, [=, this](QXmpp::SendResult result) {
                        if (const auto error = std::get_if<QXmppError>(&result)) {
                            Q_EMIT Kaidan::instance()->passiveNotificationRequested(tr("Reactions could not be sent: "
                                                                                       "%1")
                                                                                        .arg(error->description));
...
void MessageModel::handleMessageUpdated(const Message &message)
{
    for (int i = 0; i < m_messages.length(); i++) {
        ...
        if ((!oldId.isEmpty() && oldId == message.id) || (!oldId.isEmpty() && oldId == message.replaceId)
...
int MessageModel::searchForMessageFromNewToOld(const QString &searchString, int startIndex)
{
    ...
    if (foundIndex < m_messages.size()) {
        ...
        await(MessageDb::instance()->fetchMessagesUntilQueryString(AccountManager::instance()->jid(), m_currentChatJid, foundIndex, searchString),
              this,
              [this](auto result) {
    ...
    if (itr != m_messages.cend()) {
        ...
        if (lastReadContactMessageId == messageId || lastReadOwnMessageId == messageId) {
            ...
            const QString newLastReadMessageId{
                isNewLastReadMessageIdValid && newLastReadMessageIndex < m_messages.length() ? m_messages.at(newLastReadMessageIndex).id : QString()};
...
void MessageModel::showMessageNotification(const Message &message, MessageOrigin origin) const
{
    ...
    if (!message.isOwn()) {
        ...
        bool chatActive = isChatCurrentChat(accountJid, chatJid) && QGuiApplication::applicationState() == Qt::ApplicationActive;
