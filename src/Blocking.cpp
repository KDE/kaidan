...
#include "Kaidan.h"
#include "SqlUtils.h"
#include "kaidan_blocking_debug.h"
...
void BlockingController::handleXmppBlocklistResult(QXmppBlockingManager::BlocklistResult &&result)
{
    if (auto error = std::get_if<QXmppError>(&result)) {
        ...
        qCDebug(KAIDAN_BLOCKING_LOG) << "Error fetching blocklist:" << error->description;
    } else {
        // success
...
{
    qCDebug(KAIDAN_BLOCKING_LOG) << "Blocking" << jid;
    ...
    callRemoteTask(
        ...
        [this, jid](auto result) {
            if (auto *error = std::get_if<QXmppError>(&result)) {
                ...
                qCDebug(KAIDAN_BLOCKING_LOG) << "Blocked" << jid;
...
{
    qCDebug(KAIDAN_BLOCKING_LOG) << "Unblocking" << jid;
    setRunning(m_running + 1);
    callRemoteTask(
        Kaidan::instance()->client(),
        [this, jid]() {
        ...
        [this, jid](auto result) {
            if (auto *error = std::get_if<QXmppError>(&result)) {
                ...
                qCDebug(KAIDAN_BLOCKING_LOG) << "Unblocked" << jid;
