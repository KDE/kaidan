...
auto TrustDb::keys(const QString &encryption, TrustLevels trustLevels) -> QXmppTask<QHash<TrustLevel, QMultiHash<QString, QByteArray>>>
{
    ...
    -> QXmppTask<QHash<QString, QHash<QByteArray, TrustLevel>>>
{
    ...
    return runTask([this, encryption, trustLevels, keyOwnerJids] {
        ...
        auto parseQuery = [](QSqlQuery &query) {
            std::vector<QueryResult> keys;
            while (query.next()) {
                keys.push_back(QueryResult{query.value(KeyId).toByteArray(), query.value(TrustLevel_).value<TrustLevel>()});
            }
            return keys;
        };
...
auto TrustDb::addKeys(const QString &encryption, const QString &keyOwnerJid, const QList<QByteArray> &keyIds, TrustLevel trustLevel) -> QXmppTask<void>
{
    std::vector<Key> keys;
...
auto TrustDb::trustLevel(const QString &encryption, const QString &keyOwnerJid, const QByteArray &keyId) -> QXmppTask<TrustLevel>
{
...
auto TrustDb::_trustLevel(Database *database, const QString &accountJid, const QString &encryption, const QString &keyOwnerJid, const QByteArray &keyId)
    -> QXmpp::TrustLevel
{
...
auto TrustDb::setTrustLevel(const QString &encryption, const QMultiHash<QString, QByteArray> &keyIds, TrustLevel trustLevel) -> QXmppTask<TrustChanges>
{
    return runTask([this, encryption, keyIds, trustLevel, account = accountJid()] {
        enum {
            RowId,
            TrustLevel_,
        };
...
{
    return runTask([this, encryption, keyOwnerJids, oldTrustLevel, newTrustLevel, account = accountJid()] {
        TrustChanges result;
        auto &changes = result[encryption];
                                                   ...
                                                   const QList<QByteArray> &keyIdsForDistrusting) -> QXmppTask<void>
{
    ...
    std::transform(keyIdsForAuthentication.begin(), keyIdsForAuthentication.end(), std::back_inserter(selectors), [toSelector](const auto &keyId) {
        return toSelector(true, keyId);
    });
    std::transform(keyIdsForDistrusting.begin(), keyIdsForDistrusting.end(), std::back_inserter(selectors), [toSelector](const auto &keyId) {
        return toSelector(false, keyId);
    });
...
auto TrustDb::removeKeysForPostponedTrustDecisions(const QString &encryption, const QList<QByteArray> &senderKeyIds) -> QXmppTask<void>
{
    ...
    -> QXmppTask<QHash<bool, QMultiHash<QString, QByteArray>>>
{
    if (senderKeyIds.isEmpty()) {
        return runTask([this, encryption] {
            ...
            while (query.next()) {
                result[query.value(Trust).toBool()].insert(query.value(OwnerJid).toString(), query.value(KeyId).toByteArray());
            }
...
QString TrustDb::accountJid() const
{
