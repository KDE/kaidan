...
public:
	DbConnection() : m_name(QString::number(QRandomGenerator::global()->generate(), 36))
	{
...
void Database::createV3Database()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("Roster",
			SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(lastExchanged, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
					unreadMessages, SQL_INTEGER) SQL_ATTRIBUTE(lastMessage, SQL_TEXT)
					SQL_ATTRIBUTE(lastOnline, SQL_TEXT) SQL_ATTRIBUTE(activity, SQL_TEXT)
						SQL_ATTRIBUTE(status, SQL_TEXT) SQL_ATTRIBUTE(mood, SQL_TEXT)
							SQL_LAST_ATTRIBUTE(avatarHash, SQL_TEXT)));

	execQuery(query,
		SQL_CREATE_TABLE("Messages",
			SQL_ATTRIBUTE(author, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(author_resource,
				SQL_TEXT) SQL_ATTRIBUTE(recipient, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(recipient_resource,
				SQL_TEXT) SQL_ATTRIBUTE(timestamp, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(message,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(id,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(isSent, SQL_BOOL) SQL_ATTRIBUTE(isDelivered,
				SQL_BOOL) "FOREIGN KEY('author') REFERENCES Roster ('jid'),"
...
{
	QMetaObject::invokeMethod(d->dbWorker, [this] { transaction(); });
}

void Database::commitTransaction()
{
	QMetaObject::invokeMethod(d->dbWorker, [this] { commit(); });
}
...
void Database::saveDatabaseInfo()
{
	...
	execQuery(query,
		db.driver()->sqlStatement(
			QSqlDriver::UpdateStatement, QStringLiteral(DB_TABLE_INFO), updateRecord, false));
}
...
void Database::convertDatabase()
{
	...
	if (d->version == DbNotCreated) {
		...
		qDebug() << "[Database] Converting database from version" << d->version
			 << "to latest version" << DATABASE_LATEST_VERSION;
...
void Database::createNewDatabase()
{
	...
	execQuery(query,
		db.driver()->sqlStatement(
			QSqlDriver::InsertStatement, QStringLiteral(DB_TABLE_INFO), insertRecord, false));

	// accounts
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_ACCOUNTS,
			SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(
				latestMessageStanzaId, SQL_TEXT) SQL_ATTRIBUTE(latestMessageStanzaTimestamp,
				SQL_TEXT) SQL_ATTRIBUTE(httpUploadLimit, SQL_INTEGER) SQL_ATTRIBUTE(contactNotificationRule,
				SQL_INTEGER) SQL_ATTRIBUTE(groupChatNotificationRule, SQL_INTEGER)
				SQL_ATTRIBUTE(geoLocationMapPreviewEnabled, SQL_BOOL) SQL_ATTRIBUTE(
					geoLocationMapService, SQL_INTEGER) "PRIMARY KEY(jid)"));

	// roster
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_ROSTER,
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				name, SQL_TEXT) SQL_ATTRIBUTE(subscription, SQL_INTEGER) SQL_ATTRIBUTE(groupChatParticipantId,
				SQL_TEXT) SQL_ATTRIBUTE(groupChatName, SQL_TEXT) SQL_ATTRIBUTE(groupChatDescription,
				SQL_TEXT) SQL_ATTRIBUTE(groupChatFlags, SQL_INTEGER) SQL_ATTRIBUTE(encryption,
				SQL_INTEGER) SQL_ATTRIBUTE(unreadMessages, SQL_INTEGER) SQL_ATTRIBUTE(lastReadOwnMessageId,
				SQL_TEXT) SQL_ATTRIBUTE(lastReadContactMessageId, SQL_TEXT) SQL_ATTRIBUTE(latestGroupChatMessageStanzaId,
				SQL_TEXT) SQL_ATTRIBUTE(latestGroupChatMessageStanzaTimestamp, SQL_TEXT)
				SQL_ATTRIBUTE(readMarkerPending, SQL_BOOL) SQL_ATTRIBUTE(pinningPosition,
					SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(chatStateSendingEnabled,
					SQL_BOOL) SQL_ATTRIBUTE(readMarkerSendingEnabled, SQL_BOOL)
					SQL_ATTRIBUTE(notificationRule, SQL_INTEGER) SQL_ATTRIBUTE(automaticMediaDownloadsRule,
						SQL_INTEGER) "PRIMARY KEY(accountJid, jid)"));
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_ROSTER_GROUPS,
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				chatJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name,
				SQL_TEXT_NOT_NULL) "PRIMARY KEY(accountJid, chatJid, name),"
						   "FOREIGN KEY(accountJid) "
						   "REFERENCES " DB_TABLE_ROSTER " (accountJid),"
						   "FOREIGN KEY(chatJid) "
						   "REFERENCES " DB_TABLE_ROSTER " (jid)"));

	// messages
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_MESSAGES,
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(isOwn, SQL_BOOL_NOT_NULL) SQL_ATTRIBUTE(groupChatSenderId,
				SQL_TEXT) SQL_ATTRIBUTE(id, SQL_TEXT) SQL_ATTRIBUTE(originId,
				SQL_TEXT) SQL_ATTRIBUTE(stanzaId, SQL_TEXT) SQL_ATTRIBUTE(replaceId,
				SQL_TEXT) SQL_ATTRIBUTE(replyTo, SQL_TEXT) SQL_ATTRIBUTE(replyId,
				SQL_TEXT) SQL_ATTRIBUTE(replyQuote, SQL_TEXT) SQL_ATTRIBUTE(timestamp,
				SQL_TEXT) SQL_ATTRIBUTE(body, SQL_TEXT) SQL_ATTRIBUTE(encryption,
				SQL_INTEGER) SQL_ATTRIBUTE(senderKey, SQL_BLOB) SQL_ATTRIBUTE(deliveryState,
				SQL_INTEGER) SQL_ATTRIBUTE(isSpoiler, SQL_BOOL) SQL_ATTRIBUTE(spoilerHint,
				SQL_TEXT) SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER) SQL_ATTRIBUTE(groupChatInviterJid,
				SQL_TEXT) SQL_ATTRIBUTE(groupChatInviteeJid, SQL_TEXT)
				SQL_ATTRIBUTE(groupChatInvitationJid, SQL_TEXT) SQL_ATTRIBUTE(groupChatToken,
					SQL_TEXT) SQL_ATTRIBUTE(errorText, SQL_TEXT) SQL_ATTRIBUTE(removed,
					SQL_BOOL_NOT_NULL) "FOREIGN KEY(accountJid, chatJid) "
							   "REFERENCES roster (accountJid, jid)"));
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_MESSAGE_REACTIONS,
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(messageSenderId, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(messageId, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
					senderId, SQL_TEXT) SQL_ATTRIBUTE(emoji, SQL_TEXT_NOT_NULL)
					SQL_ATTRIBUTE(timestamp, SQL_INTEGER) SQL_ATTRIBUTE(deliveryState,
						SQL_INTEGER) "PRIMARY KEY(accountJid, chatJid, "
	...
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_GROUP_CHAT_USERS,
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(id, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid,
				SQL_TEXT) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(status,
				SQL_INTEGER) "PRIMARY KEY(accountJid, chatJid, id, jid),"
					     "FOREIGN KEY(accountJid, chatJid) "
					     "REFERENCES " DB_TABLE_ROSTER " (accountJid, jid)"));

	// file sharing
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_FILES,
			SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(fileGroupId,
				SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(description,
				SQL_TEXT) SQL_ATTRIBUTE(mimeType, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(size,
				SQL_INTEGER) SQL_ATTRIBUTE(lastModified, SQL_INTEGER_NOT_NULL)
				SQL_ATTRIBUTE(disposition, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(
					thumbnail, SQL_BLOB) SQL_ATTRIBUTE(localFilePath, SQL_TEXT)
	...
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_FILE_HASHES,
			SQL_ATTRIBUTE(dataId, SQL_INTEGER_NOT_NULL)
				SQL_ATTRIBUTE(hashType, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(hashValue,
					SQL_BLOB_NOT_NULL) "PRIMARY KEY(dataId, hashType)"));
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_FILE_HTTP_SOURCES,
			SQL_ATTRIBUTE(fileId, SQL_INTEGER_NOT_NULL)
				SQL_ATTRIBUTE(url, SQL_BLOB_NOT_NULL) "PRIMARY KEY(fileId)"));
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_FILE_ENCRYPTED_SOURCES,
			SQL_ATTRIBUTE(fileId, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(url, SQL_BLOB_NOT_NULL)
				SQL_ATTRIBUTE(cipher, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(key, SQL_BLOB_NOT_NULL)
					SQL_ATTRIBUTE(iv, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(encryptedDataId,
						SQL_INTEGER) "PRIMARY KEY(fileId)"));

	// trust management
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_TRUST_SECURITY_POLICIES,
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(encryption, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(securityPolicy,
					SQL_INTEGER_NOT_NULL) "PRIMARY KEY(account, encryption)"));
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_TRUST_OWN_KEYS,
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(encryption, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(keyId,
					SQL_BLOB_NOT_NULL) "PRIMARY KEY(account, encryption)"));
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_TRUST_KEYS,
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(encryption,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(ownerJid, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(keyId, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(trustLevel,
					SQL_INTEGER_NOT_NULL) "PRIMARY KEY(account, encryption, "
							      "keyId, ownerJid)"));
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_TRUST_KEYS_UNPROCESSED,
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(encryption, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(senderKeyId, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(ownerJid, SQL_TEXT_NOT_NULL)
					SQL_ATTRIBUTE(keyId, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(trust,
						SQL_BOOL_NOT_NULL) "PRIMARY KEY(account, "
								   "encryption, keyId, ownerJid)"));

	// OMEMO data
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_OMEMO_OWN_DEVICES,
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL)
				SQL_ATTRIBUTE(label, SQL_TEXT) SQL_ATTRIBUTE(privateKey, SQL_BLOB)
					SQL_ATTRIBUTE(publicKey, SQL_BLOB) SQL_ATTRIBUTE(latestSignedPreKeyId,
						SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(latestPreKeyId,
						SQL_INTEGER_NOT_NULL) "PRIMARY KEY(account)"));
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_OMEMO_DEVICES,
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(userJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(label,
				SQL_TEXT) SQL_ATTRIBUTE(keyId, SQL_BLOB) SQL_ATTRIBUTE(session,
				SQL_BLOB) SQL_ATTRIBUTE(unrespondedStanzasSent, SQL_INTEGER " DEFAULT 0")
				SQL_ATTRIBUTE(unrespondedStanzasReceived, SQL_INTEGER " DEFAULT 0")
					SQL_ATTRIBUTE(removalTimestamp,
						SQL_INTEGER) "PRIMARY KEY(account, userJid, id)"));
	execQuery(query,
	...
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_OMEMO_SIGNED_PRE_KEY_PAIRS,
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(id, SQL_BLOB_NOT_NULL)
				SQL_ATTRIBUTE(data, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(creationTimestamp,
					SQL_INTEGER) "PRIMARY KEY(account, id)"));

	// blocked
	execQuery(query,
		SQL_CREATE_TABLE(DB_TABLE_BLOCKED,
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				jid, SQL_TEXT_NOT_NULL) "PRIMARY KEY(accountJid, jid)"));

	execQuery(query,
		QStringLiteral(
			"CREATE VIEW " DB_VIEW_CHAT_MESSAGES " AS SELECT * FROM " DB_TABLE_MESSAGES
			" WHERE deliveryState != 4 AND removed != 1"));
	execQuery(query,
		QStringLiteral("CREATE VIEW " DB_VIEW_DRAFT_MESSAGES
			       " AS SELECT * FROM " DB_TABLE_MESSAGES " WHERE deliveryState = 4"));
...
void Database::convertDatabaseToV4()
{
	...
	QSqlQuery query(currentDatabase());
	// SQLite doesn't support the ALTER TABLE drop columns feature, so we have to use a
	// workaround. we copy all rows into a back-up table (but without `avatarHash`), and then
	// delete the old table and copy everything to the normal table again
	execQuery(query,
	...
	execQuery(query,
		QStringLiteral(
			"CREATE TABLE Roster (jid " SQL_TEXT_NOT_NULL ", name " SQL_TEXT_NOT_NULL ","
			"lastExchanged " SQL_TEXT_NOT_NULL ", unreadMessages " SQL_INTEGER
			", lastMessage  " SQL_TEXT ","
			"lastOnline " SQL_TEXT ", activity " SQL_TEXT ", status " SQL_TEXT
			", mood " SQL_TEXT ")"));
	execQuery(query,
		QStringLiteral(
...
void Database::convertDatabaseToV6()
{
	...
	QSqlQuery query(currentDatabase());
	const QStringView newColumns[] { u"mediaSize " SQL_INTEGER,
...
void Database::convertDatabaseToV8()
{
	...
	execQuery(query,
		QStringLiteral("INSERT INTO roster_backup SELECT jid, name, lastExchanged, "
			       "unreadMessages, lastMessage FROM Roster"));
	execQuery(query, QStringLiteral("DROP TABLE Roster"));
	execQuery(query,
		SQL_CREATE_TABLE("Roster",
			SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(
				lastExchanged, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(unreadMessages, SQL_INTEGER)
				SQL_LAST_ATTRIBUTE(lastMessage, SQL_TEXT_NOT_NULL)));

	execQuery(query,
		QStringLiteral("INSERT INTO Roster SELECT jid, name, lastExchanged, "
			       "unreadMessages, lastMessage FROM Roster_backup"));
	execQuery(query, QStringLiteral("DROP TABLE roster_backup"));
	d->version = 8;
}
...
void Database::convertDatabaseToV15()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("trust_security_policies",
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(encryption, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(security_policy,
					SQL_INTEGER_NOT_NULL) "PRIMARY KEY(account, encryption)"));
	execQuery(query,
		SQL_CREATE_TABLE("trust_own_keys",
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(encryption, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(key_id,
					SQL_BLOB_NOT_NULL) "PRIMARY KEY(account, encryption)"));
	execQuery(query,
		SQL_CREATE_TABLE("trust_keys",
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(encryption,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(key_id, SQL_BLOB_NOT_NULL)
				SQL_ATTRIBUTE(owner_jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(trust_level,
					SQL_INTEGER_NOT_NULL) "PRIMARY KEY(account, encryption, "
							      "key_id, owner_jid)"));
	execQuery(query,
		SQL_CREATE_TABLE("trust_keys_unprocessed",
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(encryption, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				key_id, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(owner_jid, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(sender_key_id, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(trust,
					SQL_BOOL_NOT_NULL) "PRIMARY KEY(account, encryption, "
							   "key_id, owner_jid)"));
	d->version = 15;
}
...
void Database::convertDatabaseToV16()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("omemo_devices_own",
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL)
				SQL_ATTRIBUTE(label, SQL_TEXT) SQL_ATTRIBUTE(private_key, SQL_BLOB)
					SQL_ATTRIBUTE(public_key, SQL_BLOB) SQL_ATTRIBUTE(latest_signed_pre_key_id,
						SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(latest_pre_key_id,
						SQL_INTEGER_NOT_NULL) "PRIMARY KEY(account)"));
	execQuery(query,
		SQL_CREATE_TABLE("omemo_devices",
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(user_jid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(label,
				SQL_TEXT) SQL_ATTRIBUTE(key_id, SQL_BLOB) SQL_ATTRIBUTE(session,
				SQL_BLOB) SQL_ATTRIBUTE(unresponded_stanzas_sent, SQL_INTEGER " DEFAULT 0")
				SQL_ATTRIBUTE(unresponded_stanzas_received, SQL_INTEGER " DEFAULT 0")
					SQL_ATTRIBUTE(removal_timestamp,
						SQL_INTEGER) "PRIMARY KEY(account, user_jid, id)"));
	execQuery(query,
	...
	execQuery(query,
		SQL_CREATE_TABLE("omemo_pre_key_pairs_signed",
			SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				id, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(data, SQL_BLOB_NOT_NULL)
...
void Database::convertDatabaseToV18()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("messages_tmp",
			SQL_ATTRIBUTE(sender, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(recipient, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				timestamp, SQL_TEXT) SQL_ATTRIBUTE(message, SQL_TEXT) SQL_ATTRIBUTE(id,
				SQL_TEXT) SQL_ATTRIBUTE(encryption, SQL_INTEGER) SQL_ATTRIBUTE(senderKey,
				SQL_BLOB) SQL_ATTRIBUTE(deliveryState, SQL_INTEGER) SQL_ATTRIBUTE(isMarkable,
				SQL_BOOL) SQL_ATTRIBUTE(mediaUrl, SQL_TEXT) SQL_ATTRIBUTE(mediaLocation,
				SQL_TEXT) SQL_ATTRIBUTE(isEdited, SQL_BOOL) SQL_ATTRIBUTE(spoilerHint,
				SQL_TEXT) SQL_ATTRIBUTE(isSpoiler, SQL_BOOL) SQL_ATTRIBUTE(errorText,
				SQL_TEXT) SQL_ATTRIBUTE(replaceId, SQL_TEXT) SQL_ATTRIBUTE(originId,
				SQL_TEXT) SQL_ATTRIBUTE(stanzaId, SQL_TEXT) SQL_ATTRIBUTE(file_group_id,
				SQL_INTEGER) "FOREIGN KEY(sender) REFERENCES Roster (jid),"
					     "FOREIGN KEY(recipient) REFERENCES Roster (jid)"));

	execQuery(query,
		QStringLiteral(
			"INSERT INTO messages_tmp SELECT author, recipient, timestamp, message, "
			"id, encryption, "
			"senderKey, deliveryState, isMarkable, mediaUrl, mediaLocation, edited, "
			"spoilerHint, "
...
void Database::convertDatabaseToV19()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("files",
			SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(file_group_id,
				SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(description,
				SQL_TEXT) SQL_ATTRIBUTE(mime_type, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(size,
				SQL_INTEGER) SQL_ATTRIBUTE(last_modified, SQL_INTEGER_NOT_NULL)
				SQL_ATTRIBUTE(disposition, SQL_INTEGER_NOT_NULL)
					SQL_ATTRIBUTE(thumbnail, SQL_BLOB) SQL_ATTRIBUTE(
						local_file_path, SQL_TEXT) "PRIMARY KEY(id)"));
	execQuery(query,
		SQL_CREATE_TABLE("file_hashes",
			SQL_ATTRIBUTE(data_id, SQL_INTEGER_NOT_NULL)
				SQL_ATTRIBUTE(hash_type, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(hash_value,
					SQL_BLOB_NOT_NULL) "PRIMARY KEY(data_id, hash_type)"));
	execQuery(query,
		SQL_CREATE_TABLE("file_http_sources",
			SQL_ATTRIBUTE(file_id, SQL_INTEGER_NOT_NULL)
				SQL_ATTRIBUTE(url, SQL_BLOB_NOT_NULL) "PRIMARY KEY(file_id)"));
	execQuery(query,
		SQL_CREATE_TABLE("file_encrypted_sources",
			SQL_ATTRIBUTE(file_id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(url, SQL_BLOB_NOT_NULL)
				SQL_ATTRIBUTE(cipher, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(key, SQL_BLOB_NOT_NULL)
					SQL_ATTRIBUTE(iv, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(encrypted_data_id,
						SQL_INTEGER) "PRIMARY KEY(file_id)"));
	...
	execQuery(query,
		SQL_CREATE_TABLE("messages_tmp",
			SQL_ATTRIBUTE(sender, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(recipient, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				...
				SQL_TEXT) SQL_ATTRIBUTE(encryption, SQL_INTEGER) SQL_ATTRIBUTE(senderKey,
				SQL_BLOB) SQL_ATTRIBUTE(deliveryState, SQL_INTEGER) SQL_ATTRIBUTE(isMarkable,
				SQL_BOOL) SQL_ATTRIBUTE(isEdited, SQL_BOOL) SQL_ATTRIBUTE(spoilerHint,
				SQL_TEXT) SQL_ATTRIBUTE(isSpoiler, SQL_BOOL) SQL_ATTRIBUTE(errorText,
				SQL_TEXT) SQL_ATTRIBUTE(replaceId, SQL_TEXT) SQL_ATTRIBUTE(originId,
				SQL_TEXT) SQL_ATTRIBUTE(stanzaId, SQL_TEXT) SQL_ATTRIBUTE(file_group_id,
				SQL_INTEGER) "FOREIGN KEY(sender) REFERENCES Roster (jid),"
	...
	execQuery(query,
		QStringLiteral("INSERT INTO messages_tmp SELECT sender, recipient, timestamp, "
			       "message, id, encryption, "
			       "senderKey, deliveryState, isMarkable, isEdited, spoilerHint, "
			       "isSpoiler, errorText, "
...
void Database::convertDatabaseToV22()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("messageReactions",
			SQL_ATTRIBUTE(messageSender, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(messageRecipient,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(messageId, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(senderJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(timestamp, SQL_INTEGER) SQL_ATTRIBUTE(emoji,
				SQL_TEXT_NOT_NULL) "PRIMARY KEY(messageSender, messageRecipient, "
						   "messageId, senderJid, timestamp, emoji)"));
...
void Database::convertDatabaseToV23()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("roster_tmp",
			SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(
				subscription, SQL_INTEGER) SQL_ATTRIBUTE(encryption, SQL_INTEGER)
				SQL_ATTRIBUTE(unreadMessages, SQL_INTEGER) SQL_ATTRIBUTE(lastReadOwnMessageId,
					SQL_TEXT) SQL_ATTRIBUTE(lastReadContactMessageId, SQL_TEXT)
					SQL_ATTRIBUTE(readMarkerPending, SQL_BOOL)
						SQL_LAST_ATTRIBUTE(pinningPosition, SQL_INTEGER)));

	execQuery(query,
		QStringLiteral("INSERT INTO roster_tmp SELECT jid, name, subscription, encryption, "
			       "unreadMessages, "
			       "lastReadOwnMessageId, lastReadContactMessageId, readMarkerPending, "
			       "pinningPosition "
			       " FROM Roster"));
	...
	execQuery(query,
		QStringLiteral("ALTER TABLE omemoDevices RENAME COLUMN "
			       "unresponded_stanzas_received TO unrespondedStanzasReceived"));
...
void Database::convertDatabaseToV25()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("messages_tmp",
			SQL_ATTRIBUTE(sender, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(recipient,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(timestamp, SQL_TEXT) SQL_ATTRIBUTE(message,
				SQL_TEXT) SQL_ATTRIBUTE(id, SQL_TEXT) SQL_ATTRIBUTE(encryption,
				SQL_INTEGER) SQL_ATTRIBUTE(senderKey, SQL_BLOB) SQL_ATTRIBUTE(deliveryState,
				SQL_INTEGER) SQL_ATTRIBUTE(isEdited, SQL_BOOL) SQL_ATTRIBUTE(spoilerHint,
				SQL_TEXT) SQL_ATTRIBUTE(isSpoiler, SQL_BOOL) SQL_ATTRIBUTE(errorText,
				SQL_TEXT) SQL_ATTRIBUTE(replaceId, SQL_TEXT) SQL_ATTRIBUTE(originId,
				SQL_TEXT) SQL_ATTRIBUTE(stanzaId, SQL_TEXT) SQL_ATTRIBUTE(fileGroupId,
				SQL_INTEGER) "FOREIGN KEY(sender) REFERENCES roster (jid),"
					     "FOREIGN KEY(recipient) REFERENCES roster (jid)"));
	...
	execQuery(query,
		QStringLiteral("INSERT INTO messages_tmp SELECT sender, recipient, timestamp, "
			       "message, id, encryption, "
			       "senderKey, deliveryState, isEdited, spoilerHint, isSpoiler, "
			       "errorText, replaceId, "
...
void Database::convertDatabaseToV26()
{
		...
		SQL_CREATE_TABLE("roster_tmp",
			SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(
				...
				SQL_ATTRIBUTE(unreadMessages, SQL_INTEGER) SQL_ATTRIBUTE(lastReadOwnMessageId,
					SQL_TEXT) SQL_ATTRIBUTE(lastReadContactMessageId, SQL_TEXT)
					SQL_ATTRIBUTE(readMarkerPending, SQL_BOOL) SQL_ATTRIBUTE(pinningPosition,
						SQL_INTEGER) SQL_ATTRIBUTE(chatStateSendingEnabled, SQL_BOOL)
	...
	execQuery(query,
		SQL_CREATE_TABLE("roster",
			SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(subscription,
				SQL_INTEGER) SQL_ATTRIBUTE(encryption, SQL_INTEGER) SQL_ATTRIBUTE(unreadMessages,
				SQL_INTEGER) SQL_ATTRIBUTE(lastReadOwnMessageId, SQL_TEXT)
				SQL_ATTRIBUTE(lastReadContactMessageId, SQL_TEXT) SQL_ATTRIBUTE(readMarkerPending,
					SQL_BOOL) SQL_ATTRIBUTE(pinningPosition, SQL_INTEGER_NOT_NULL)
...
void Database::convertDatabaseToV29()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("messageReactions_tmp",
			SQL_ATTRIBUTE(messageSender, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(messageRecipient,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(messageId, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(senderJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(emoji, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(timestamp, SQL_INTEGER) SQL_ATTRIBUTE(deliveryState,
					SQL_INTEGER) "PRIMARY KEY(messageSender, messageRecipient, "
						     "messageId, senderJid, emoji)"));

	execQuery(query,
		QStringLiteral("INSERT INTO messageReactions_tmp SELECT messageSender, "
...
void Database::convertDatabaseToV32()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("messages_tmp",
			SQL_ATTRIBUTE(sender, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(recipient,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(timestamp, SQL_TEXT) SQL_ATTRIBUTE(body,
				SQL_TEXT) SQL_ATTRIBUTE(id, SQL_TEXT) SQL_ATTRIBUTE(encryption,
				SQL_INTEGER) SQL_ATTRIBUTE(senderKey, SQL_BLOB) SQL_ATTRIBUTE(deliveryState,
				SQL_INTEGER) SQL_ATTRIBUTE(spoilerHint, SQL_TEXT) SQL_ATTRIBUTE(isSpoiler,
				SQL_BOOL) SQL_ATTRIBUTE(errorText, SQL_TEXT) SQL_ATTRIBUTE(replaceId,
				SQL_TEXT) SQL_ATTRIBUTE(originId, SQL_TEXT) SQL_ATTRIBUTE(stanzaId,
				SQL_TEXT) SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER) SQL_ATTRIBUTE(removed,
				SQL_BOOL_NOT_NULL) "FOREIGN KEY(sender) REFERENCES roster (jid),"
						   "FOREIGN KEY(recipient) REFERENCES roster "
						   "(jid)"));

	execQuery(query,
		QStringLiteral(
			"INSERT INTO messages_tmp SELECT sender, recipient, timestamp, body, id, "
			"encryption, "
...
void Database::convertDatabaseToV33()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("roster",
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(subscription,
				SQL_INTEGER) SQL_ATTRIBUTE(encryption, SQL_INTEGER) SQL_ATTRIBUTE(unreadMessages,
				SQL_INTEGER) SQL_ATTRIBUTE(lastReadOwnMessageId, SQL_TEXT) SQL_ATTRIBUTE(lastReadContactMessageId,
				SQL_TEXT) SQL_ATTRIBUTE(readMarkerPending, SQL_BOOL) SQL_ATTRIBUTE(pinningPosition,
				SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(chatStateSendingEnabled,
				SQL_BOOL) SQL_ATTRIBUTE(readMarkerSendingEnabled,
				SQL_BOOL) SQL_ATTRIBUTE(draftMessageId, SQL_TEXT) SQL_ATTRIBUTE(notificationsMuted,
				SQL_BOOL) "PRIMARY KEY(accountJid, jid),"
	...
	execQuery(query,
		SQL_CREATE_TABLE("rosterGroups",
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				chatJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name,
				SQL_TEXT_NOT_NULL) "PRIMARY KEY(accountJid, chatJid, name),"
						   "FOREIGN KEY(accountJid) REFERENCES roster "
						   "(accountJid),"
	...
	execQuery(query,
		SQL_CREATE_TABLE("roster_tmp",
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid,
				...
				SQL_INTEGER) SQL_ATTRIBUTE(lastReadOwnMessageId, SQL_TEXT) SQL_ATTRIBUTE(lastReadContactMessageId,
				SQL_TEXT) SQL_ATTRIBUTE(readMarkerPending, SQL_BOOL) SQL_ATTRIBUTE(pinningPosition,
				SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(chatStateSendingEnabled, SQL_BOOL)
				SQL_ATTRIBUTE(readMarkerSendingEnabled, SQL_BOOL) SQL_ATTRIBUTE(notificationsMuted,
					SQL_BOOL) "PRIMARY KEY(accountJid, jid)"));

	execQuery(query,
		QStringLiteral("INSERT INTO roster_tmp SELECT accountJid, jid, name, subscription, "
			       "encryption, unreadMessages, "
			       "lastReadOwnMessageId, lastReadContactMessageId, readMarkerPending, "
			       "pinningPosition, "
			       "chatStateSendingEnabled, readMarkerSendingEnabled, "
			       "notificationsMuted FROM roster"));
...
void Database::convertDatabaseToV36()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("messages",
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				senderId, SQL_TEXT) SQL_ATTRIBUTE(timestamp, SQL_TEXT) SQL_ATTRIBUTE(body,
				SQL_TEXT) SQL_ATTRIBUTE(id, SQL_TEXT) SQL_ATTRIBUTE(encryption,
				SQL_INTEGER) SQL_ATTRIBUTE(senderKey, SQL_BLOB) SQL_ATTRIBUTE(deliveryState,
				SQL_INTEGER) SQL_ATTRIBUTE(spoilerHint, SQL_TEXT) SQL_ATTRIBUTE(isSpoiler,
				SQL_BOOL) SQL_ATTRIBUTE(errorText, SQL_TEXT) SQL_ATTRIBUTE(replaceId,
				SQL_TEXT) SQL_ATTRIBUTE(originId, SQL_TEXT) SQL_ATTRIBUTE(stanzaId, SQL_TEXT)
				SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER) SQL_ATTRIBUTE(removed,
					SQL_BOOL_NOT_NULL) "FOREIGN KEY(accountJid, chatJid) "
							   "REFERENCES roster (accountJid, jid)"));
	...
	execQuery(query,
		SQL_CREATE_TABLE("messageReactions",
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(messageSenderId, SQL_TEXT_NOT_NULL)
				SQL_ATTRIBUTE(messageId, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(senderJid,
					SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(emoji, SQL_TEXT_NOT_NULL)
					SQL_ATTRIBUTE(timestamp, SQL_INTEGER) SQL_ATTRIBUTE(deliveryState,
						SQL_INTEGER) "PRIMARY KEY(accountJid, chatJid, "
...
void Database::convertDatabaseToV37()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("messages_tmp",
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(senderId, SQL_TEXT) SQL_ATTRIBUTE(id,
				SQL_TEXT) SQL_ATTRIBUTE(originId, SQL_TEXT) SQL_ATTRIBUTE(stanzaId,
				SQL_TEXT) SQL_ATTRIBUTE(replaceId, SQL_TEXT) SQL_ATTRIBUTE(timestamp,
				SQL_TEXT) SQL_ATTRIBUTE(body, SQL_TEXT) SQL_ATTRIBUTE(encryption,
				SQL_INTEGER) SQL_ATTRIBUTE(senderKey, SQL_BLOB) SQL_ATTRIBUTE(deliveryState,
				SQL_INTEGER) SQL_ATTRIBUTE(isSpoiler, SQL_BOOL) SQL_ATTRIBUTE(spoilerHint,
				SQL_TEXT) SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER)
				SQL_ATTRIBUTE(errorText, SQL_TEXT) SQL_ATTRIBUTE(removed,
					SQL_BOOL_NOT_NULL) "FOREIGN KEY(accountJid, chatJid) "
							   "REFERENCES roster (accountJid, jid)"));

	execQuery(query,
		QStringLiteral("INSERT INTO messages_tmp SELECT accountJid, chatJid, senderId, id, "
			       "originId, stanzaId, "
			       "replaceId, timestamp, body, encryption, senderKey, deliveryState, "
			       "isSpoiler, spoilerHint, "
...
void Database::convertDatabaseToV40()
{
	...
	execQuery(query,
		SQL_CREATE_TABLE("accounts",
			SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT)
				SQL_ATTRIBUTE(latestMessageStanzaId, SQL_TEXT) SQL_ATTRIBUTE(
					latestMessageTimestamp, SQL_TEXT) "PRIMARY KEY(jid)"));
...
void Database::convertDatabaseToV43()
{
		...
		SQL_CREATE_TABLE("roster_tmp",
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(
				...
				SQL_ATTRIBUTE(readMarkerPending, SQL_BOOL) SQL_ATTRIBUTE(pinningPosition,
					SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(chatStateSendingEnabled,
					SQL_BOOL) SQL_ATTRIBUTE(readMarkerSendingEnabled, SQL_BOOL)
					SQL_ATTRIBUTE(notificationsMuted, SQL_BOOL) SQL_ATTRIBUTE(automaticMediaDownloadsRule,
						SQL_INTEGER) "PRIMARY KEY(accountJid, jid)"));

	execQuery(query,
		"INSERT INTO roster_tmp SELECT accountJid, jid, name, subscription, NULL, NULL, "
		"NULL, "
		"NULL, encryption, unreadMessages, lastReadOwnMessageId, lastReadContactMessageId, "
		"NULL, "
		"NULL, readMarkerPending, pinningPosition, chatStateSendingEnabled, "
		"readMarkerSendingEnabled, notificationsMuted, automaticMediaDownloadsRule FROM "
		"roster");
	...
	execQuery(query,
		SQL_CREATE_TABLE("messages_tmp",
			SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid,
				SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(isOwn, SQL_BOOL) SQL_ATTRIBUTE(groupChatSenderId,
				SQL_TEXT) SQL_ATTRIBUTE(id, SQL_TEXT) SQL_ATTRIBUTE(originId,
				SQL_TEXT) SQL_ATTRIBUTE(stanzaId, SQL_TEXT) SQL_ATTRIBUTE(replaceId,
				SQL_TEXT) SQL_ATTRIBUTE(timestamp, SQL_TEXT) SQL_ATTRIBUTE(body,
				SQL_TEXT) SQL_ATTRIBUTE(encryption, SQL_INTEGER) SQL_ATTRIBUTE(senderKey,
				SQL_BLOB) SQL_ATTRIBUTE(deliveryState, SQL_INTEGER) SQL_ATTRIBUTE(isSpoiler,
				SQL_BOOL) SQL_ATTRIBUTE(spoilerHint, SQL_TEXT) SQL_ATTRIBUTE(fileGroupId,
				SQL_INTEGER) SQL_ATTRIBUTE(groupChatInviterJid, SQL_TEXT)
				SQL_ATTRIBUTE(groupChatInviteeJid, SQL_TEXT) SQL_ATTRIBUTE(
					groupChatInvitationJid, SQL_TEXT) SQL_ATTRIBUTE(groupChatToken,
					SQL_TEXT) SQL_ATTRIBUTE(errorText, SQL_TEXT) SQL_ATTRIBUTE(removed,
					SQL_BOOL_NOT_NULL) "FOREIGN KEY(accountJid, chatJid) "
							   "REFERENCES roster (accountJid, jid)"));
	...
	execQuery(query,
		QStringLiteral("INSERT INTO messageReactions_tmp SELECT accountJid, chatJid, "
			       "messageSenderId, messageId, "
