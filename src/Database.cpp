...
// Building blocks for schema definitions, used by the migrations and by createNewDatabase().
...
{
    if (d->version <= DbOldVersion || d->version > latestVersion()) {
...
void Database::convertDatabase()
{
    ...
    if (d->version == DbNotCreated) {
        qCDebug(KAIDAN_CORE_LOG) << "Creating new database with latest version" << latestVersion();
        createNewDatabase();
    } else {
        qCDebug(KAIDAN_CORE_LOG) << "Converting database from version" << d->version << "to latest version" << latestVersion();

        for (const auto &migration : migrations()) {
            if (d->version < migration.version) {
                QSqlQuery query(currentDatabase());
                migration.apply(query);
                d->version = migration.version;
            }
        }
    }
...
void Database::createNewDatabase()
{
    ...
    insertRecord.append(createSqlField(QStringLiteral("version"), latestVersion()));
    ...
    execQuery(query,
              SQL_CREATE_TABLE(
                  DB_TABLE_CHATS,
                  SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(origin, SQL_INTEGER)
                      SQL_ATTRIBUTE(name, SQL_TEXT) SQL_ATTRIBUTE(subscription, SQL_INTEGER) SQL_ATTRIBUTE(subscriptionStatus, SQL_TEXT)
                          SQL_ATTRIBUTE(subscriptionApproved, SQL_BOOL) SQL_ATTRIBUTE(groupChatParticipantId, SQL_TEXT) SQL_ATTRIBUTE(groupChatName, SQL_TEXT)
                              SQL_ATTRIBUTE(groupChatDescription, SQL_TEXT) SQL_ATTRIBUTE(groupChatFlags, SQL_INTEGER) SQL_ATTRIBUTE(encryption, SQL_INTEGER)
                                  SQL_ATTRIBUTE(lastReadOwnMessageId, SQL_TEXT) SQL_ATTRIBUTE(lastReadContactMessageId, SQL_TEXT)
                                      SQL_ATTRIBUTE(latestGroupChatMessageStanzaId, SQL_TEXT) SQL_ATTRIBUTE(latestGroupChatMessageStanzaTimestamp, SQL_TEXT)
              ...
              SQL_CREATE_TABLE(DB_TABLE_ROSTER_GROUPS,
                               SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid, SQL_TEXT_NOT_NULL)
                                   SQL_ATTRIBUTE(name, SQL_TEXT_NOT_NULL) "PRIMARY KEY(accountJid, chatJid, name),"
                                                                          "FOREIGN KEY(accountJid) REFERENCES " DB_TABLE_CHATS " (accountJid),"
                                                                          "FOREIGN KEY(chatJid) REFERENCES " DB_TABLE_CHATS " (jid)"));
                               ...
                               SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(id, SQL_TEXT_NOT_NULL)
                                   SQL_ATTRIBUTE(jid, SQL_TEXT) SQL_ATTRIBUTE(name, SQL_TEXT)
                                       SQL_ATTRIBUTE(status, SQL_INTEGER) "PRIMARY KEY(accountJid, chatJid, id, jid),"
                                                                          "FOREIGN KEY(accountJid, chatJid) REFERENCES " DB_TABLE_CHATS " (accountJid, jid)"));
...
std::span<const Database::Migration> Database::migrations()
{
    // Add a new migration by appending an entry to this list; nothing else needs to
    // be updated.
    static constexpr Migration MIGRATIONS[] = {
         ...
         [](QSqlQuery &query) {
             ...
             execQuery(query,
                       SQL_CREATE_TABLE("omemo_devices",
                                        SQL_ATTRIBUTE(account, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(user_jid, SQL_TEXT_NOT_NULL)
                                            SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(label, SQL_TEXT) SQL_ATTRIBUTE(key_id, SQL_BLOB)
                                                SQL_ATTRIBUTE(session, SQL_BLOB) SQL_ATTRIBUTE(unresponded_stanzas_sent, SQL_INTEGER " DEFAULT 0")
         ...
         [](QSqlQuery &query) {
             ...
             execQuery(query,
                       SQL_CREATE_TABLE("file_http_sources",
                                        SQL_ATTRIBUTE(file_id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(url, SQL_BLOB_NOT_NULL) "PRIMARY KEY(file_id)"));
             execQuery(query,
                       SQL_CREATE_TABLE("file_encrypted_sources",
                                        SQL_ATTRIBUTE(file_id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(url, SQL_BLOB_NOT_NULL)
                                            SQL_ATTRIBUTE(cipher, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(key, SQL_BLOB_NOT_NULL)
                                                SQL_ATTRIBUTE(iv, SQL_BLOB_NOT_NULL) SQL_ATTRIBUTE(encrypted_data_id, SQL_INTEGER) "PRIMARY KEY(file_id)"));
         }},
        {20,
         [](QSqlQuery &query) {
             ...
             execQuery(
                 ...
                 SQL_CREATE_TABLE(
                     ...
                     SQL_ATTRIBUTE(messageSender, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(messageRecipient, SQL_TEXT_NOT_NULL)
                         SQL_ATTRIBUTE(messageId, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(senderJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(timestamp, SQL_INTEGER)
                       ...
                       SQL_CREATE_TABLE("messages_tmp",
                                        SQL_ATTRIBUTE(sender, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(recipient, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(timestamp, SQL_TEXT)
                                            SQL_ATTRIBUTE(body, SQL_TEXT) SQL_ATTRIBUTE(id, SQL_TEXT) SQL_ATTRIBUTE(encryption, SQL_INTEGER)
                                                SQL_ATTRIBUTE(senderKey, SQL_BLOB) SQL_ATTRIBUTE(deliveryState, SQL_INTEGER)
                                                    SQL_ATTRIBUTE(spoilerHint, SQL_TEXT) SQL_ATTRIBUTE(isSpoiler, SQL_BOOL) SQL_ATTRIBUTE(errorText, SQL_TEXT)
                                                        SQL_ATTRIBUTE(replaceId, SQL_TEXT) SQL_ATTRIBUTE(originId, SQL_TEXT) SQL_ATTRIBUTE(stanzaId, SQL_TEXT)
        ...
        {35,
         [](QSqlQuery &query) {
             ...
             execQuery(query,
                       SQL_CREATE_TABLE("roster_tmp",
                                        SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT)
                                            SQL_ATTRIBUTE(subscription, SQL_INTEGER) SQL_ATTRIBUTE(encryption, SQL_INTEGER)
                                                SQL_ATTRIBUTE(unreadMessages, SQL_INTEGER) SQL_ATTRIBUTE(lastReadOwnMessageId, SQL_TEXT)
                                                    SQL_ATTRIBUTE(lastReadContactMessageId, SQL_TEXT) SQL_ATTRIBUTE(readMarkerPending, SQL_BOOL)
         ...
         [](QSqlQuery &query) {
             ...
             execQuery(query,
                       SQL_CREATE_TABLE("blocked",
                                        SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) "PRIMARY KEY(accountJid, jid)"));
         }},
        {40,
         [](QSqlQuery &query) {
             execQuery(query,
         ...
         [](QSqlQuery &query) {
             ...
             execQuery(query,
                       SQL_CREATE_TABLE(
                           "roster_tmp",
                           SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT)
                               SQL_ATTRIBUTE(subscription, SQL_INTEGER) SQL_ATTRIBUTE(groupChatParticipantId, SQL_TEXT) SQL_ATTRIBUTE(groupChatName, SQL_TEXT)
             ...
             execQuery(query,
                       SQL_CREATE_TABLE(
                           ...
                           SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(isOwn, SQL_BOOL)
                               SQL_ATTRIBUTE(groupChatSenderId, SQL_TEXT) SQL_ATTRIBUTE(id, SQL_TEXT) SQL_ATTRIBUTE(originId, SQL_TEXT)
                                   SQL_ATTRIBUTE(stanzaId, SQL_TEXT) SQL_ATTRIBUTE(replaceId, SQL_TEXT) SQL_ATTRIBUTE(timestamp, SQL_TEXT)
                                       SQL_ATTRIBUTE(body, SQL_TEXT) SQL_ATTRIBUTE(encryption, SQL_INTEGER) SQL_ATTRIBUTE(senderKey, SQL_BLOB)
                                           SQL_ATTRIBUTE(deliveryState, SQL_INTEGER) SQL_ATTRIBUTE(isSpoiler, SQL_BOOL) SQL_ATTRIBUTE(spoilerHint, SQL_TEXT)
                                               SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER) SQL_ATTRIBUTE(groupChatInviterJid, SQL_TEXT)
                                                   SQL_ATTRIBUTE(groupChatInviteeJid, SQL_TEXT) SQL_ATTRIBUTE(groupChatInvitationJid, SQL_TEXT)
                                                       SQL_ATTRIBUTE(groupChatToken, SQL_TEXT) SQL_ATTRIBUTE(errorText, SQL_TEXT) SQL_ATTRIBUTE(
                                                           removed,
                                                           SQL_BOOL_NOT_NULL) "FOREIGN KEY(accountJid, chatJid) REFERENCES roster (accountJid, jid)"));
			...
			WHERE accountJid = senderId
             ...
             execQuery(query,
                       SQL_CREATE_TABLE("groupChatUsers",
                                        SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid, SQL_TEXT_NOT_NULL)
                                            SQL_ATTRIBUTE(id, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT) SQL_ATTRIBUTE(name, SQL_TEXT)
                                                SQL_ATTRIBUTE(status, SQL_INTEGER) "PRIMARY KEY(accountJid, chatJid, id, jid),"
         ...
         [](QSqlQuery &query) {
             ...
             execQuery(query,
                       SQL_CREATE_TABLE(
                           ...
                           SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(chatJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(isOwn, SQL_BOOL)
                               SQL_ATTRIBUTE(groupChatSenderId, SQL_TEXT) SQL_ATTRIBUTE(id, SQL_TEXT) SQL_ATTRIBUTE(originId, SQL_TEXT)
                                   SQL_ATTRIBUTE(stanzaId, SQL_TEXT) SQL_ATTRIBUTE(replaceId, SQL_TEXT) SQL_ATTRIBUTE(replyTo, SQL_TEXT)
                                       SQL_ATTRIBUTE(replyId, SQL_TEXT) SQL_ATTRIBUTE(replyQuote, SQL_TEXT) SQL_ATTRIBUTE(timestamp, SQL_TEXT)
                                           SQL_ATTRIBUTE(body, SQL_TEXT) SQL_ATTRIBUTE(encryption, SQL_INTEGER) SQL_ATTRIBUTE(senderKey, SQL_BLOB)
                                               SQL_ATTRIBUTE(deliveryState, SQL_INTEGER) SQL_ATTRIBUTE(isSpoiler, SQL_BOOL) SQL_ATTRIBUTE(spoilerHint, SQL_TEXT)
                                                   SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER) SQL_ATTRIBUTE(groupChatInviterJid, SQL_TEXT)
                                                       SQL_ATTRIBUTE(groupChatInviteeJid, SQL_TEXT) SQL_ATTRIBUTE(groupChatInvitationJid, SQL_TEXT)
         ...
         [](QSqlQuery &query) {
             ...
             // Import account settings from settings file
             {
                 ...
                 const auto settingsAccount = [&]() -> std::optional<AccountSettings::Data> {
                     ...
                     account.passwordVisibility =
                         settings->value<AccountSettings::PasswordVisibility>(PasswordVisibility, AccountSettings::PasswordVisibility::Visible);
         ...
         [](QSqlQuery &query) {
             ...
             for (auto i = accounts.size() - 1; i >= 0; --i) {
                 ...
                 if (auto decodedPassword = QByteArray::fromBase64Encoding(account.password.toUtf8())) {
                     execQuery(query,
                               QStringLiteral("UPDATE accounts SET password = '%1' WHERE jid = '%2'").arg(QString::fromUtf8(*decodedPassword), account.jid));
                 }
             }
         }},
        {51,
                                                   ...
                                                   SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER) SQL_ATTRIBUTE(groupChatInviterJid, SQL_TEXT)
                                                       SQL_ATTRIBUTE(groupChatInviteeJid, SQL_TEXT) SQL_ATTRIBUTE(groupChatInvitationJid, SQL_TEXT)
                                                           SQL_ATTRIBUTE(groupChatToken, SQL_TEXT) SQL_ATTRIBUTE(errorText, SQL_TEXT)
                                                               SQL_ATTRIBUTE(marked, SQL_BOOL_NOT_NULL) SQL_ATTRIBUTE(
                                                                   removed,
                                                                   SQL_BOOL_NOT_NULL) "FOREIGN KEY(accountJid, chatJid) REFERENCES roster (accountJid, jid)"));
        ...
        {57,
         [](QSqlQuery &query) {
             // Add the columns "width" and "height".
             execQuery(query,
                       SQL_CREATE_TABLE("files_tmp",
                                        SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT)
                                            SQL_ATTRIBUTE(description, SQL_TEXT) SQL_ATTRIBUTE(mimeType, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(size, SQL_INTEGER)
                                                SQL_ATTRIBUTE(width, SQL_INTEGER) SQL_ATTRIBUTE(height, SQL_INTEGER)
			...
			SELECT id, fileGroupId, name, description, mimeType, size, NULL, NULL,
			lastModified, disposition, thumbnail, localFilePath, externalId, transferOutgoing, transferState
			FROM files
		)"));
             ...
             execQuery(query,
                       SQL_CREATE_TABLE("files_tmp",
                                        SQL_ATTRIBUTE(id, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(fileGroupId, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT)
                                            SQL_ATTRIBUTE(description, SQL_TEXT) SQL_ATTRIBUTE(mimeType, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(size, SQL_INTEGER)
                                                SQL_ATTRIBUTE(width, SQL_INTEGER) SQL_ATTRIBUTE(height, SQL_INTEGER)
                                                    SQL_ATTRIBUTE(lastModified, SQL_INTEGER_NOT_NULL) SQL_ATTRIBUTE(disposition, SQL_INTEGER)
                                                        SQL_ATTRIBUTE(thumbnail, SQL_BLOB) SQL_ATTRIBUTE(localFilePath, SQL_TEXT)
                                                            SQL_ATTRIBUTE(externalId, SQL_TEXT) SQL_ATTRIBUTE(transferOutgoing, SQL_BOOL_NOT_NULL)
                                                                SQL_ATTRIBUTE(transferState, SQL_INTEGER_NOT_NULL) "PRIMARY KEY(id)"));
         ...
         [](QSqlQuery &query) {
             ...
             execQuery(query,
                       SQL_CREATE_TABLE(
                           "roster_tmp",
                           SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(name, SQL_TEXT)
                               SQL_ATTRIBUTE(subscription, SQL_INTEGER) SQL_ATTRIBUTE(subscriptionStatus, SQL_TEXT)
                                   SQL_ATTRIBUTE(subscriptionApproved, SQL_BOOL) SQL_ATTRIBUTE(groupChatParticipantId, SQL_TEXT)
        ...
        {60,
         [](QSqlQuery &query) {
             execQuery(query, QStringLiteral("ALTER TABLE roster RENAME TO chats"));

             // Add the column "origin".
             execQuery(query,
                       SQL_CREATE_TABLE(
                           "chats_tmp",
                           SQL_ATTRIBUTE(accountJid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(jid, SQL_TEXT_NOT_NULL) SQL_ATTRIBUTE(origin, SQL_INTEGER)
             ...
             execQuery(query, QStringLiteral(R"(
			INSERT INTO chats_tmp
			SELECT accountJid, jid, 0, name, subscription, subscriptionStatus, subscriptionApproved, groupChatParticipantId, groupChatName,
			groupChatDescription, groupChatFlags, encryption, lastReadOwnMessageId,
			lastReadContactMessageId, latestGroupChatMessageStanzaId,
			latestGroupChatMessageStanzaTimestamp, readMarkerPending, pinningPosition,
			chatStateSendingEnabled, readMarkerSendingEnabled, notificationRule,
			automaticMediaDownloadsRule
			FROM chats
		)"));
    ...
    static_assert(std::ranges::adjacent_find(MIGRATIONS, std::greater_equal{}, &Migration::version) == std::ranges::end(MIGRATIONS),
                  "Migrations must be ordered by strictly increasing version - did a merge duplicate a version number?");

    return MIGRATIONS;
}

int Database::latestVersion()
{
