    ...
    , m_mamManager(client->findExtension<QXmppMamManager>())
{
    ...
    connect(client->findExtension<QXmppRosterManager>(), &QXmppRosterManager::rosterReceived, this, &MessageHandler::handleRosterReceived);

    connect(&m_receiptManager, &QXmppMessageReceiptManager::messageDelivered, this, [](const QString &, const QString &id) {
    ...
    if (m_client->state() == QXmppClient::ConnectedState) {
        ...
        await(send(message.toQXmpp()), this, [messageId](QXmpp::SendResult result) {
            if (const auto error = std::get_if<QXmppError>(&result)) {
                qWarning() << "[client] [MessageHandler] Could not send message:" << error->description;
...
void MessageHandler::retrieveInitialMessage(const QString &jid, const QString &offsetMessageId)
{
    ...
    m_mamManager->retrieveMessages(QString(), QString(), jid, QDateTime(), QDateTime(), queryLimit).then(this, [this, jid](Mam::RetrieveResult result) {
...
void MessageHandler::handleMessage(const QXmppMessage &msg, MessageOrigin origin)
{
    ...
    if (auto encryptionName = msg.encryptionName(); !encryptionName.isEmpty() && message.encryption == Encryption::NoEncryption) {
    ...
    if (msg.replaceId().isEmpty()) {
        ...
        message.timestamp = (msg.stamp().isNull() || !msg.stamp().isValid()) ? QDateTime::currentDateTimeUtc() : msg.stamp().toUTC();
...
bool MessageHandler::handleReaction(const QXmppMessage &message, const QString &senderJid)
{
    if (const auto receivedReaction = message.reaction()) {
        MessageDb::instance()->updateMessage(message.reaction()->messageId(),
                                             [senderJid, receivedEmojis = receivedReaction->emojis(), receivedTimestamp = message.stamp()](Message &m) {
                                                 ...
                                                 if (reactionSender.latestTimestamp.isNull() || reactionSender.latestTimestamp < receivedTimestamp) {
                                                     ...
                                                     for (const auto &receivedEmoji : std::as_const(receivedEmojis)) {
                                                         const auto reactionNew =
                                                             std::none_of(reactions.begin(), reactions.end(), [&](const MessageReaction &reaction) {
