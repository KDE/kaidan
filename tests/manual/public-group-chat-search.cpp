...
int main(int argc, char *argv[])
{
	...
	while (!(groups = listEntries(locale, nm, all.isEmpty() ? QString() : all.constLast().address))
			.isEmpty()) {
		all.append(groups);
		QThread::msleep(250);
	}
	...
	qWarning() << "Got" << all.count() << "entries occupying"
		   << locale.formattedDataSize(data.size()) << "uncompressed bytes,"
