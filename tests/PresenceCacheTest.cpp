...
void PresenceCacheTest::presenceGetter_data()
{
    ...
    QTest::newRow("bob1") << "bob@kaidan.im" << "dev1" << true;
    QTest::newRow("bob2") << "bob@kaidan.im" << "dev2" << true;
    QTest::newRow("alice1") << "alice@kaidan.im" << "kdn1" << true;
    QTest::newRow("alice2") << "alice@kaidan.im" << "kdn2" << true;
    QTest::newRow("unknwon presence") << "alice@kaidan.im" << "device3" << false;
}
...
#define ROW(name, presences, expectedResource) QTest::newRow(name) << (presences) << QStringLiteral(expectedResource)
    ...
    ROW("DND",
        QList<QXmppPresence>() << simplePresence("a@b/1", QXmppPresence::Away) << simplePresence("a@b/2", QXmppPresence::XA)
                               << simplePresence("a@b/3", QXmppPresence::Online) << simplePresence("a@b/4", QXmppPresence::Chat)
                               << simplePresence("a@b/5", QXmppPresence::DND),
        "5");
    ROW("Chat",
        QList<QXmppPresence>() << simplePresence("a@b/1", QXmppPresence::Away) << simplePresence("a@b/2", QXmppPresence::XA)
                               << simplePresence("a@b/3", QXmppPresence::Chat) << simplePresence("a@b/4", QXmppPresence::Online),
        "3");
    ROW("XA", QList<QXmppPresence>() << simplePresence("a@b/1", QXmppPresence::XA) << simplePresence("a@b/2", QXmppPresence::Away), "1");
    ROW("Away", QList<QXmppPresence>() << simplePresence("a@b/1", QXmppPresence::Away) << simplePresence("a@b/2", QXmppPresence::XA), "1");
    ROW("status text [DND]",
        QList<QXmppPresence>() << simplePresence("a@b/1", QXmppPresence::Away) << simplePresence("a@b/2", QXmppPresence::XA)
                               << simplePresence("a@b/3", QXmppPresence::Online) << simplePresence("a@b/4", QXmppPresence::Chat)
                               << simplePresence("a@b/5", QXmppPresence::DND) << simplePresence("a@b/6", QXmppPresence::Chat, "my status")
                               << simplePresence("a@b/7", QXmppPresence::DND, "my status") << simplePresence("a@b/8", QXmppPresence::Away, "my status"),
        "7");
    ROW("status text [XA]",
        QList<QXmppPresence>() << simplePresence("a@b/1", QXmppPresence::Away) << simplePresence("a@b/2", QXmppPresence::XA)
                               << simplePresence("a@b/3", QXmppPresence::XA, "my status") << simplePresence("a@b/4", QXmppPresence::Away, "my status"),
        "3");
