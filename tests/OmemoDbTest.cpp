...
{
	return a.id == b.id && a.label == b.label && a.latestPreKeyId == b.latestPreKeyId &&
	       a.latestSignedPreKeyId == b.latestSignedPreKeyId &&
...
{
	return a.label == b.label && a.keyId == b.keyId && a.session == b.session &&
	       a.unrespondedReceivedStanzasCount == b.unrespondedReceivedStanzasCount &&
	       a.unrespondedSentStanzasCount == b.unrespondedSentStanzasCount &&
	       a.removalFromDeviceListDate == b.removalFromDeviceListDate;
}
...
private:
	...
	Database db;
	OmemoDb storage = OmemoDb(&db, "user@example.org", this);
};

void OmemoDbTest::testOwnDevice()
{
	QVERIFY(!wait(storage.allData()).ownDevice.has_value());

	Storage::OwnDevice ownDevice;
	storage.setOwnDevice(ownDevice);

	// Check the default values.
	auto data = wait(storage.allData());
	...
	ownDevice = Storage::OwnDevice {
		...
		.label = ("Notebook"),
		.privateIdentityKey =
			QByteArray::fromBase64("ZDVNZFdJeFFUa3N6ZWdSUG9scUdoQXFpWERGbHRsZTIK"),
		.publicIdentityKey =
			QByteArray::fromBase64("dUsxSTJyM2tKVHE1TzNXbk1Xd0tpMGY0TnFleDRYUGkK"),
	...
	QCOMPARE(wait(storage.allData()).ownDevice, ownDevice);
}

void OmemoDbTest::testSignedPreKeyPairs()
{
	Storage::SignedPreKeyPairs signedPreKeyPairs;
	QCOMPARE(wait(storage.allData()).signedPreKeyPairs, signedPreKeyPairs);
...
void OmemoDbTest::testPreKeyPairs()
{
	...
	QCOMPARE(wait(storage.allData()).preKeyPairs, preKeyPairs);
...
{
	// empty check
	QCOMPARE(wait(storage.allData()).devices, Storage::Devices());
	...
	deviceBob1.session = QByteArray::fromBase64(
		("CvgCCAQSIQXZwE+G9R6ECMxKWPMidwcx3lPboUT2KEoea3B2T3vjUBohBQ7qW+Fb9Gi/"
	...
	deviceBob2.session = QByteArray::fromBase64(
		("CvgCCAQSIQU/tpDP8FUNlK2WTyQUZhmT9GaAglohE+ysQN4ngGvPPBohBdnAT4b1HoQI"
	...
	{
		auto result = wait(storage.allData()).devices;
		...
		QCOMPARE(resultDeviceAlice.keyId,
			QByteArray::fromBase64(("bEFLaDRQRkFlYXdyakE2aURoN0wyMzk2NTJEM2hRMgo=")));
		QCOMPARE(resultDeviceAlice.session,
			QByteArray::fromBase64(
				("Cs8CCAQSIQWIhBRMdJ80tLVT7ius0H1LutRLeXBid68NH90M/"
				 "kwhGxohBT+2kM/"
		...
		QCOMPARE(resultDeviceBob1.keyId,
			QByteArray::fromBase64(("WTV6c3B2UFhYbE9OQ1d0N0ZScUhLWXpmYnY2emJoego=")));
		...
		QCOMPARE(resultDeviceBob2.keyId,
			QByteArray::fromBase64(("U0tXcUlSVHVISzZLYUdGcW53czBtdXYxTEt2blVsbQo=")));
