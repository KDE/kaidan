...
void TrustDbTest::testKeys()
{
    ...
    QVERIFY(!wait(this, storage.hasKey(ns_omemo, "alice@example.org", TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated)));
    ...
    std::vector<Key> keys = {
        Key{ns_omemo, "alice@example.org", QByteArray::fromBase64("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ="), TrustLevel::ManuallyDistrusted},
        Key{ns_omemo, "alice@example.org", QByteArray::fromBase64("Ciemp4ZNzRJxnRD+k28vAie0kXJrwl4IrbfDy7n6OxE="), TrustLevel::AutomaticallyTrusted},
        Key{ns_omemo, "bob@example.com", QByteArray::fromBase64("rvSXBRd+EICMhQvVgcREQJxxP+T4EBmai4mYHBfJQGg="), TrustLevel::AutomaticallyTrusted},
        Key{ns_omemo, "bob@example.com", QByteArray::fromBase64("2fhJtrgoMJxfLI3084/YkYh9paqiSiLFDVL2m0qAgX4="), TrustLevel::ManuallyTrusted},
        Key{ns_omemo, "bob@example.com", QByteArray::fromBase64("tCP1CI3pqSTVGzFYFyPYUMfMZ9Ck/msmfD0wH/VtJBM="), TrustLevel::ManuallyTrusted},
        Key{ns_omemo, "bob@example.com", QByteArray::fromBase64("YjVI04NcbTPvXLaA95RO84HPcSvyOgEZ2r5cTyUs0C8="), TrustLevel::Authenticated},
        Key{ns_ox, "alice@example.org", QByteArray::fromBase64("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ="), TrustLevel::Authenticated},
        Key{ns_ox, "alice@example.org", QByteArray::fromBase64("IhpPjiKLchgrAG5cpSfTvdzPjZ5v6vTOluHEUehkgCA="), TrustLevel::Authenticated},
    };
    ...
    QMultiHash<QString, QByteArray> manuallyDistrustedKeys = {{"alice@example.org", QByteArray::fromBase64(("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ="))}};
    QMultiHash<QString, QByteArray> automaticallyTrustedKeys = {{"alice@example.org", QByteArray::fromBase64(("Ciemp4ZNzRJxnRD+k28vAie0kXJrwl4IrbfDy7n6OxE="))},
                                                                {"bob@example.com", QByteArray::fromBase64(("rvSXBRd+EICMhQvVgcREQJxxP+T4EBmai4mYHBfJQGg="))}};
    QMultiHash<QString, QByteArray> manuallyTrustedKeys = {{"bob@example.com", QByteArray::fromBase64(("tCP1CI3pqSTVGzFYFyPYUMfMZ9Ck/msmfD0wH/VtJBM="))},
                                                           {"bob@example.com", QByteArray::fromBase64(("2fhJtrgoMJxfLI3084/YkYh9paqiSiLFDVL2m0qAgX4="))}};
    QMultiHash<QString, QByteArray> authenticatedKeys = {{"bob@example.com", QByteArray::fromBase64(("YjVI04NcbTPvXLaA95RO84HPcSvyOgEZ2r5cTyUs0C8="))}};
    ...
    QCOMPARE(wait(this, storage.keys(ns_omemo, {"alice@example.org", "bob@example.com"})),
             TrustDb::KeysByOwner({std::pair("alice@example.org", keysAlice), std::pair("bob@example.com", keysBob)}));

    // Alice's OMEMO keys
    QCOMPARE(wait(this, storage.keys(ns_omemo, {"alice@example.org"})), TrustDb::KeysByOwner({std::pair("alice@example.org", keysAlice)}));
    ...
    QCOMPARE(wait(this, storage.keys(ns_omemo, {"alice@example.org", "bob@example.com"}, TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated)),
             TrustDb::KeysByOwner({std::pair("alice@example.org", keysAlice), std::pair("bob@example.com", keysBob)}));
    ...
    automaticallyDistrustedKeys = {{"alice@example.org", QByteArray::fromBase64(("/1eK3R2LtjPBT3el8f0q4DvzqUJSfFy5fkKkKPNFNYw="))}};
    automaticallyTrustedKeys = {{"bob@example.com", QByteArray::fromBase64(("rvSXBRd+EICMhQvVgcREQJxxP+T4EBmai4mYHBfJQGg="))}};
    ...
    QVERIFY(wait(this, storage.keys(ns_omemo, {"alice@example.org"}, TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated)).isEmpty());
    ...
    authenticatedKeys = {{"alice@example.org", QByteArray::fromBase64(("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ="))},
    ...
    QCOMPARE(wait(this, storage.keys(ns_ox, {"alice@example.org", "bob@example.com"})), TrustDb::KeysByOwner({std::pair("alice@example.org", keysAlice)}));

    // Alice's remaining OX keys
    QCOMPARE(wait(this, storage.keys(ns_ox, {"alice@example.org"})), TrustDb::KeysByOwner({{"alice@example.org", keysAlice}}));
    ...
    auto hasKeyResult = wait(this, storage.hasKey(ns_ox, "alice@example.org", TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated));
    QVERIFY(!hasKeyResult);
}

void TrustDbTest::testTrustLevels()
{
    storage.addKeys(ns_ox, "alice@example.org", {QByteArray::fromBase64(("AZ/cF4OrUOILKO1gQBf62pQevOhBJ2NyHnXLwM4FDZU="))}, TrustLevel::AutomaticallyTrusted);
    ...
    storage.addKeys(ns_omemo, "alice@example.org", {QByteArray::fromBase64(("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ="))}, TrustLevel::ManuallyTrusted);

    storage.addKeys(ns_omemo, "bob@example.com", {QByteArray::fromBase64(("9E51lG3vVmUn8CM7/AIcmIlLP2HPl6Ao0/VSf4VT/oA="))}, TrustLevel::AutomaticallyTrusted);

    QCOMPARE(wait(this, storage.trustLevel(ns_omemo, "alice@example.org", QByteArray::fromBase64(("AZ/cF4OrUOILKO1gQBf62pQevOhBJ2NyHnXLwM4FDZU=")))),
             TrustLevel::AutomaticallyTrusted);
    ...
    QCOMPARE(int(wait(this, storage.trustLevel(ns_omemo, "alice@example.org", QByteArray::fromBase64("AZ/cF4OrUOILKO1gQBf62pQevOhBJ2NyHnXLwM4FDZU=")))),
             int(TrustLevel::Authenticated));

    QCOMPARE(wait(this, storage.trustLevel(ns_omemo, "bob@example.com", QByteArray::fromBase64(("9E51lG3vVmUn8CM7/AIcmIlLP2HPl6Ao0/VSf4VT/oA=")))),
             TrustLevel::Authenticated);
    ...
    QCOMPARE(int(wait(this, storage.trustLevel(ns_omemo, "alice@example.org", QByteArray::fromBase64("9w6oPjKyGSALd9gHq7sNOdOAkD5bHUVOKACNs89FjkA=")))),
             int(TrustLevel::ManuallyTrusted));

    // Try to retrieve the trust level of a key that is not stored yet.
    // The default value is returned.
    QCOMPARE(wait(this, storage.trustLevel(ns_omemo, "someone@example.org", QByteArray::fromBase64("WXL4EDfzUGbVPQWjT9pmBeiCpCBzYZv3lUAaj+UbPyE="))),
             TrustLevel::Undecided);

    // Set the trust levels of all authenticated keys belonging to Alice and
    // Bob.
    storage.setTrustLevel(ns_omemo, {"alice@example.org", "bob@example.com"}, TrustLevel::Authenticated, TrustLevel::ManuallyDistrusted);
    ...
    QCOMPARE(wait(this, storage.trustLevel(ns_omemo, "somebody@example.org", QByteArray::fromBase64(("wE06Gwf8f4DvDLFDoaCsGs8ibcUjf84WIOA2FAjPI3o=")))),
             TrustLevel::Undecided);
...
void TrustDbTest::testResetAll()
{
    ...
    storage.addKeys(ns_omemo, "alice@example.org", {QByteArray::fromBase64(("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ="))}, TrustLevel::ManuallyDistrusted);
    ...
    storage.addKeys(ns_omemo, "bob@example.com", {QByteArray::fromBase64(("rvSXBRd+EICMhQvVgcREQJxxP+T4EBmai4mYHBfJQGg="))}, TrustLevel::AutomaticallyTrusted);
    ...
    storage.addKeys(ns_omemo, "bob@example.com", {QByteArray::fromBase64(("YjVI04NcbTPvXLaA95RO84HPcSvyOgEZ2r5cTyUs0C8="))}, TrustLevel::Authenticated);
    ...
    const QMultiHash<QString, QByteArray> authenticatedKeys = {{"alice@example.org", QByteArray::fromBase64("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ=")},
                                                               {"alice@example.org", QByteArray::fromBase64("IhpPjiKLchgrAG5cpSfTvdzPjZ5v6vTOluHEUehkgCA=")}};
...
void TrustDbTest::atmTestKeysForPostponedTrustDecisions()
{
    ...
    QMultiHash<QString, QByteArray> trustedKeys = {{"alice@example.org", QByteArray::fromBase64(("Wl53ZchbtAtCZQCHROiD20W7UnKTQgWQrjTHAVNw1ic="))},
    ...
    QMultiHash<QString, QByteArray> distrustedKeys = {{"alice@example.org", QByteArray::fromBase64(("mB98hhdVps++skUuy4TGy/Vp6RQXLJO4JGf86FAUjyc="))},
    ...
    QCOMPARE(wait(this,
                  storage.keysForPostponedTrustDecisions(ns_omemo,
                                                         {QByteArray::fromBase64(("Mp6Y4wOF3aMcl38lb/"
                                                                                  "VNbdPF9ucGFqSx2eyaEsqyHKE="))})),
    ...
    trustedKeys = {{"bob@example.com", QByteArray::fromBase64(("sD6ilugEBeKxPsdDEyX43LSGKHKWd5MFEdhT+4RpsxA="))}};
    distrustedKeys = {{"bob@example.com", QByteArray::fromBase64(("X5tJ1D5rEeaeQE8eqhBKAj4KUZGYe3x+iHifaTBY1kM="))}};
    ...
    trustedKeys = {{"alice@example.org", QByteArray::fromBase64(("QR05jrab7PFkSLhtdzyXrPfCqhkNCYCrlWATaBMTenE="))}};
...
void TrustDbTest::atmTestResetAll()
{
    ...
    QMultiHash<QString, QByteArray> trustedKeys = {{("carol@example.net"), QByteArray::fromBase64(("WcL+cEMpEeK+dpqg3Xd3amctzwP8h2MqwXcEzFf6LpU="))},
                                                   {("carol@example.net"), QByteArray::fromBase64(("bH3R31z0N97K1fUwG3+bdBrVPuDfXguQapHudkfa5nE="))}};
    QMultiHash<QString, QByteArray> distrustedKeys = {{("carol@example.net"), QByteArray::fromBase64(("N0B2StHKk1/slwg1rzybTFzjdg7FChc+3cXmTU/rS8g="))},
                                                      {("carol@example.net"), QByteArray::fromBase64(("wsEN32UHCiNjYqTG/J63hY4Nu8tZT42Ni1FxrgyRQ5g="))}};
