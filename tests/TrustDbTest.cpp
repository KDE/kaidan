...
private:
	Database db;
	TrustDb storage = TrustDb(&db, "account@example.org");
};

void TrustDbTest::testSecurityPolicy()
{
	QCOMPARE(wait(storage.securityPolicy(ns_ox)), QXmpp::NoSecurityPolicy);
	storage.setSecurityPolicy(ns_omemo, QXmpp::Toakafa);

	QCOMPARE(wait(storage.securityPolicy(ns_ox)), QXmpp::NoSecurityPolicy);
	QCOMPARE(wait(storage.securityPolicy(ns_omemo)), QXmpp::Toakafa);

	storage.resetSecurityPolicy(ns_omemo);
	QCOMPARE(wait(storage.securityPolicy(ns_omemo)), QXmpp::NoSecurityPolicy);
}

void TrustDbTest::testOwnKeys()
{
	QVERIFY(wait(storage.ownKey(ns_ox)).isNull());
	...
	// own OX key
	QCOMPARE(wait(storage.ownKey(ns_ox)),
		QByteArray::fromBase64(("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ=")));

	// own OMEMO key
	QCOMPARE(wait(storage.ownKey(ns_omemo)),
	...
	// no own OMEMO key
	QVERIFY(wait(storage.ownKey(ns_omemo)).isEmpty());
}

void TrustDbTest::testKeys()
{
	// no OMEMO keys
	QVERIFY(wait(storage.keys(ns_omemo)).isEmpty());

	// no OMEMO keys (via JIDs)
	QVERIFY(wait(storage.keys(ns_omemo, {"alice@example.org", "bob@example.com"})).isEmpty());

	// no automatically trusted and authenticated OMEMO keys
	QVERIFY(wait(storage.keys(ns_omemo, TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated))
			.isEmpty());

	// no automatically trusted and authenticated OMEMO key from Alice
	QVERIFY(!wait(storage.hasKey(
	...
	auto addKey = [this](const Key &key) {
		wait(storage.addKeys(key.encryption, key.jid, {key.key}, key.trust));
	};
	std::vector<Key> keys = {
		Key {ns_omemo,
			"alice@example.org",
	...
	// all OMEMO keys
	auto omemoKeys = wait(storage.keys(ns_omemo));
	...
	QCOMPARE(wait(storage.keys(ns_omemo, TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated)),
	...
	QCOMPARE(wait(storage.keys(ns_omemo, {"alice@example.org", "bob@example.com"})),
		TrustDb::KeysByOwner({std::pair("alice@example.org", keysAlice),
			std::pair("bob@example.com", keysBob)}));

	// Alice's OMEMO keys
	QCOMPARE(wait(storage.keys(ns_omemo, {"alice@example.org"})),
		TrustDb::KeysByOwner({std::pair("alice@example.org", keysAlice)}));
	...
	QCOMPARE(wait(storage.keys(ns_omemo,
	...
	QCOMPARE(wait(storage.keys(ns_omemo, {"alice@example.org"}, TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated)),
		TrustDb::KeysByOwner({std::pair("alice@example.org", keysAlice)}));
	...
	QVERIFY(wait(storage.keys(ns_omemo, {"alice@example.org"}, TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated))
			.isEmpty());
	...
	// remaining OX keys
	QCOMPARE(wait(storage.keys(ns_ox)), QHash({std::pair(TrustLevel::Authenticated, authenticatedKeys)}));

	// remaining OX keys (via JIDs)
	QCOMPARE(wait(storage.keys(ns_ox, {"alice@example.org", "bob@example.com"})),
		TrustDb::KeysByOwner({std::pair("alice@example.org", keysAlice)}));

	// Alice's remaining OX keys
	QCOMPARE(wait(storage.keys(ns_ox, {"alice@example.org"})),
		TrustDb::KeysByOwner({{"alice@example.org", keysAlice}}));

	storage.removeKeys(ns_ox);

	// no stored OX keys
	QVERIFY(wait(storage.keys(ns_ox)).isEmpty());

	// no stored OX keys (via JIDs)
	QVERIFY(wait(storage.keys(ns_ox, {"alice@example.org", "bob@example.com"})).isEmpty());

	// no automatically trusted or authenticated OX key from Alice
	auto hasKeyResult = wait(storage.hasKey(
...
void TrustDbTest::testTrustLevels()
{
	...
	QCOMPARE(wait(storage.trustLevel(ns_omemo,
			 "alice@example.org",
			 QByteArray::fromBase64(("AZ/cF4OrUOILKO1gQBf62pQevOhBJ2NyHnXLwM4FDZU=")))),
		TrustLevel::AutomaticallyTrusted);
	...
	QCOMPARE(int(wait(storage.trustLevel(ns_omemo,
			 "alice@example.org",
			 QByteArray::fromBase64("AZ/cF4OrUOILKO1gQBf62pQevOhBJ2NyHnXLwM4FDZU=")))),
	...
	wait(storage.setTrustLevel(ns_omemo,
...
void TrustDbTest::testResetAll()
{
	...
	QCOMPARE(wait(storage.securityPolicy(ns_omemo)), NoSecurityPolicy);
	QCOMPARE(wait(storage.securityPolicy(ns_ox)), Toakafa);
	QVERIFY(wait(storage.ownKey(ns_omemo)).isEmpty());
	QCOMPARE(wait(storage.ownKey(ns_ox)),
		QByteArray::fromBase64("aFABnX7Q/rbTgjBySYzrT2FsYCVYb49mbca5yB734KQ="));
	QVERIFY(wait(storage.keys(ns_omemo)).isEmpty());
...
void TrustDbTest::atmTestKeysForPostponedTrustDecisions()
{
	...
	QCOMPARE(wait(storage.keysForPostponedTrustDecisions(
	...
	QCOMPARE(wait(storage.keysForPostponedTrustDecisions(ns_omemo,
			 {QByteArray::fromBase64(("Mp6Y4wOF3aMcl38lb/VNbdPF9ucGFqSx2eyaEsqyHKE=")),
	...
	QVERIFY(wait(storage.keysForPostponedTrustDecisions(ns_omemo)).isEmpty());
	...
	// remaining OX keys
	QCOMPARE(wait(storage.keysForPostponedTrustDecisions(ns_ox)),
		QHash({std::pair(true, trustedKeys), std::pair(false, distrustedKeys)}));

	storage.removeKeysForPostponedTrustDecisions(ns_ox);

	// no OX keys
	QVERIFY(wait(storage.keysForPostponedTrustDecisions(ns_ox)).isEmpty());
}
