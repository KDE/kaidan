...
void TrustDbTest::testKeys()
{
	...
	QVERIFY(!wait(this,
		storage.hasKey(ns_omemo, "alice@example.org", TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated)));
	...
	// remaining OX keys
	QCOMPARE(wait(this, storage.keys(ns_ox)),
		QHash({ std::pair(TrustLevel::Authenticated, authenticatedKeys) }));
	...
	auto hasKeyResult = wait(this,
		storage.hasKey(ns_ox, "alice@example.org", TrustLevel::AutomaticallyTrusted | TrustLevel::Authenticated));
