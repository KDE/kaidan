...
private:
    Database db;
    TrustDb storage = TrustDb(&db, this, QStringLiteral("account@example.org"));
};
...
void TrustDbTest::testTrustLevels()
{
    ...
    QCOMPARE(
        wait(this, storage.trustLevel(ns_omemo, QStringLiteral("alice@example.org"), QByteArray::fromBase64(("AZ/cF4OrUOILKO1gQBf62pQevOhBJ2NyHnXLwM4FDZU=")))),
        TrustLevel::AutomaticallyTrusted);
    ...
    QCOMPARE(
        wait(this, storage.trustLevel(ns_omemo, QStringLiteral("someone@example.org"), QByteArray::fromBase64("WXL4EDfzUGbVPQWjT9pmBeiCpCBzYZv3lUAaj+UbPyE="))),
